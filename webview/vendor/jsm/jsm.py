"""Symbolic utilities for the `webview/vendor/jsm` data bucket."""
...
__all__ = ["JsmSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "webview/vendor/jsm", "module": "jsm"},
}
...
def load_snapshot(root: Path | None = None) -> JsmSnapshot:
    ...
    if path.exists():
        ...
        return JsmSnapshot(bucket="webview/vendor/jsm", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = JsmSnapshot(bucket="webview/vendor/jsm", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> JsmSnapshot:
    ...
    snapshot = JsmSnapshot(bucket="webview/vendor/jsm", context=ctx, signals=signals)
