"""Symbolic utilities for the `webview/vendor/jsm/utils` data bucket."""
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "webview/vendor/jsm/utils", "module": "utils"},
}
...
def load_snapshot(root: Path | None = None) -> UtilsSnapshot:
    ...
    if path.exists():
        ...
        return UtilsSnapshot(bucket="webview/vendor/jsm/utils", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = UtilsSnapshot(bucket="webview/vendor/jsm/utils", context=_DEFAULT_CONTEXT, signals=signals)
...
def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> UtilsSnapshot:
    ...
    snapshot = UtilsSnapshot(bucket="webview/vendor/jsm/utils", context=ctx, signals=signals)
