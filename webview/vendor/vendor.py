"""Symbolic utilities for the `webview/vendor` data bucket."""
...
__all__ = ["VendorSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "webview/vendor", "module": "vendor"},
}
...
def load_snapshot(root: Path | None = None) -> VendorSnapshot:
    ...
    if path.exists():
        ...
        return VendorSnapshot(bucket="webview/vendor", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = VendorSnapshot(bucket="webview/vendor", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> VendorSnapshot:
    ...
    snapshot = VendorSnapshot(bucket="webview/vendor", context=ctx, signals=signals)
