"""Symbolic utilities for the `webview/vendor/three_legacy/loaders/utils` data bucket."""
...
__all__ = ["UtilsSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "webview/vendor/three_legacy/loaders/utils", "module": "utils"},
}
...
def load_snapshot(root: Path | None = None) -> UtilsSnapshot:
    ...
    if path.exists():
        ...
        return UtilsSnapshot(bucket="webview/vendor/three_legacy/loaders/utils", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = UtilsSnapshot(bucket="webview/vendor/three_legacy/loaders/utils", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> UtilsSnapshot:
    ...
    snapshot = UtilsSnapshot(bucket="webview/vendor/three_legacy/loaders/utils", context=ctx, signals=signals)
