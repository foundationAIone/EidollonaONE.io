"""Symbolic utilities for the `webview/vendor/three_legacy/loaders` data bucket."""
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "webview/vendor/three_legacy/loaders", "module": "loaders"},
}
...
def load_snapshot(root: Path | None = None) -> LoadersSnapshot:
    ...
    if path.exists():
        ...
        return LoadersSnapshot(bucket="webview/vendor/three_legacy/loaders", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = LoadersSnapshot(bucket="webview/vendor/three_legacy/loaders", context=_DEFAULT_CONTEXT, signals=signals)
...
def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> LoadersSnapshot:
    ...
    snapshot = LoadersSnapshot(bucket="webview/vendor/three_legacy/loaders", context=ctx, signals=signals)
