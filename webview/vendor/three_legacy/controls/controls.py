"""Symbolic utilities for the `webview/vendor/three_legacy/controls` data bucket."""
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "webview/vendor/three_legacy/controls", "module": "controls"},
}
...
def load_snapshot(root: Path | None = None) -> ControlsSnapshot:
    ...
    if path.exists():
        ...
        return ControlsSnapshot(bucket="webview/vendor/three_legacy/controls", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = ControlsSnapshot(bucket="webview/vendor/three_legacy/controls", context=_DEFAULT_CONTEXT, signals=signals)
...
def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> ControlsSnapshot:
    ...
    snapshot = ControlsSnapshot(bucket="webview/vendor/three_legacy/controls", context=ctx, signals=signals)
