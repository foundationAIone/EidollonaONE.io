"""Symbolic utilities for the `webview/vendor/three_legacy` data bucket."""
...
__all__ = ["ThreeLegacySnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "webview/vendor/three_legacy", "module": "three_legacy"},
}


@dataclass(slots=True)
class ThreeLegacySnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> ThreeLegacySnapshot:
    ...
    if path.exists():
        ...
        return ThreeLegacySnapshot(bucket="webview/vendor/three_legacy", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = ThreeLegacySnapshot(bucket="webview/vendor/three_legacy", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> ThreeLegacySnapshot:
    ...
    snapshot = ThreeLegacySnapshot(bucket="webview/vendor/three_legacy", context=ctx, signals=signals)
