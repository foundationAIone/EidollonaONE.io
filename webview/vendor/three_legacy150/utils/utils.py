"""Symbolic utilities for the `webview/vendor/three_legacy150/utils` data bucket."""
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "webview/vendor/three_legacy150/utils", "module": "utils"},
}
...
def load_snapshot(root: Path | None = None) -> UtilsSnapshot:
    ...
    if path.exists():
        ...
        return UtilsSnapshot(bucket="webview/vendor/three_legacy150/utils", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = UtilsSnapshot(bucket="webview/vendor/three_legacy150/utils", context=_DEFAULT_CONTEXT, signals=signals)
...
def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> UtilsSnapshot:
    ...
    snapshot = UtilsSnapshot(bucket="webview/vendor/three_legacy150/utils", context=ctx, signals=signals)
