"""Symbolic utilities for the `webview/vendor/three_legacy150` data bucket."""
...
__all__ = ["ThreeLegacy150Snapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "webview/vendor/three_legacy150", "module": "three_legacy150"},
}


@dataclass(slots=True)
class ThreeLegacy150Snapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> ThreeLegacy150Snapshot:
    ...
    if path.exists():
        ...
        return ThreeLegacy150Snapshot(bucket="webview/vendor/three_legacy150", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = ThreeLegacy150Snapshot(bucket="webview/vendor/three_legacy150", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> ThreeLegacy150Snapshot:
    ...
    snapshot = ThreeLegacy150Snapshot(bucket="webview/vendor/three_legacy150", context=ctx, signals=signals)
