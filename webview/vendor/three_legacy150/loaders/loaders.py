"""Symbolic utilities for the `webview/vendor/three_legacy150/loaders` data bucket."""
...
__all__ = ["LoadersSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "webview/vendor/three_legacy150/loaders", "module": "loaders"},
}


@dataclass(slots=True)
class LoadersSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> LoadersSnapshot:
    ...
    if path.exists():
        ...
        return LoadersSnapshot(bucket="webview/vendor/three_legacy150/loaders", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = LoadersSnapshot(bucket="webview/vendor/three_legacy150/loaders", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> LoadersSnapshot:
    ...
    snapshot = LoadersSnapshot(bucket="webview/vendor/three_legacy150/loaders", context=ctx, signals=signals)
