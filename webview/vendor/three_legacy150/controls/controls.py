"""Symbolic utilities for the `webview/vendor/three_legacy150/controls` data bucket."""
...
__all__ = ["ControlsSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "webview/vendor/three_legacy150/controls", "module": "controls"},
}
...
def load_snapshot(root: Path | None = None) -> ControlsSnapshot:
    ...
    if path.exists():
        ...
        return ControlsSnapshot(bucket="webview/vendor/three_legacy150/controls", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = ControlsSnapshot(bucket="webview/vendor/three_legacy150/controls", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> ControlsSnapshot:
    ...
    snapshot = ControlsSnapshot(bucket="webview/vendor/three_legacy150/controls", context=ctx, signals=signals)
