"""Symbolic utilities for the `webview` data bucket."""
...
__all__ = ["WebviewSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "webview", "module": "webview"},
}
...
def load_snapshot(root: Path | None = None) -> WebviewSnapshot:
    ...
    if path.exists():
        ...
        return WebviewSnapshot(bucket="webview", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = WebviewSnapshot(bucket="webview", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> WebviewSnapshot:
    ...
    snapshot = WebviewSnapshot(bucket="webview", context=ctx, signals=signals)
