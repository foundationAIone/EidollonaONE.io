"""Symbolic utilities for the `enforcement_data` data bucket."""
...
__all__ = ["EnforcementDataSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "enforcement_data", "module": "enforcement_data"},
}


@dataclass(slots=True)
class EnforcementDataSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> EnforcementDataSnapshot:
    ...
    if path.exists():
        ...
        return EnforcementDataSnapshot(bucket="enforcement_data", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = EnforcementDataSnapshot(bucket="enforcement_data", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> EnforcementDataSnapshot:
    ...
    snapshot = EnforcementDataSnapshot(bucket="enforcement_data", context=ctx, signals=signals)
