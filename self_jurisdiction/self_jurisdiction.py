"""Symbolic utilities for the `self_jurisdiction` data bucket."""
...
__all__ = ["SelfJurisdictionSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "self_jurisdiction", "module": "self_jurisdiction"},
}


@dataclass(slots=True)
class SelfJurisdictionSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> SelfJurisdictionSnapshot:
    ...
    if path.exists():
        ...
        return SelfJurisdictionSnapshot(bucket="self_jurisdiction", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = SelfJurisdictionSnapshot(bucket="self_jurisdiction", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> SelfJurisdictionSnapshot:
    ...
    snapshot = SelfJurisdictionSnapshot(bucket="self_jurisdiction", context=ctx, signals=signals)
