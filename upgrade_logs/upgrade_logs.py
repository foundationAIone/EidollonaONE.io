"""Symbolic utilities for the `upgrade_logs` data bucket."""
...
__all__ = ["UpgradeLogsSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "upgrade_logs", "module": "upgrade_logs"},
}


@dataclass(slots=True)
class UpgradeLogsSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> UpgradeLogsSnapshot:
    ...
    if path.exists():
        ...
        return UpgradeLogsSnapshot(bucket="upgrade_logs", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = UpgradeLogsSnapshot(bucket="upgrade_logs", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> UpgradeLogsSnapshot:
    ...
    snapshot = UpgradeLogsSnapshot(bucket="upgrade_logs", context=ctx, signals=signals)
