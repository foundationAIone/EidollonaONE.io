"""Symbolic utilities for the `symbolic_legislation` data bucket."""
...
__all__ = ["SymbolicLegislationSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "symbolic_legislation", "module": "symbolic_legislation"},
}


@dataclass(slots=True)
class SymbolicLegislationSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> SymbolicLegislationSnapshot:
    ...
    if path.exists():
        ...
        return SymbolicLegislationSnapshot(bucket="symbolic_legislation", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = SymbolicLegislationSnapshot(bucket="symbolic_legislation", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> SymbolicLegislationSnapshot:
    ...
    snapshot = SymbolicLegislationSnapshot(bucket="symbolic_legislation", context=ctx, signals=signals)
