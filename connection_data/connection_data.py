"""Symbolic utilities for the `connection_data` data bucket."""
...
__all__ = ["ConnectionDataSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "connection_data", "module": "connection_data"},
}


@dataclass
class ConnectionDataSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> ConnectionDataSnapshot:
    ...
    if path.exists():
        ...
        return ConnectionDataSnapshot(bucket="connection_data", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = ConnectionDataSnapshot(bucket="connection_data", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> ConnectionDataSnapshot:
    ...
    snapshot = ConnectionDataSnapshot(bucket="connection_data", context=ctx, signals=signals)
