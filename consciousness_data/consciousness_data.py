"""Symbolic utilities for the `consciousness_data` data bucket."""
...
__all__ = ["ConsciousnessDataSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "consciousness_data", "module": "consciousness_data"},
}


@dataclass()
class ConsciousnessDataSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> ConsciousnessDataSnapshot:
    ...
    if path.exists():
        ...
        return ConsciousnessDataSnapshot(bucket="consciousness_data", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = ConsciousnessDataSnapshot(bucket="consciousness_data", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> ConsciousnessDataSnapshot:
    ...
    snapshot = ConsciousnessDataSnapshot(bucket="consciousness_data", context=ctx, signals=signals)
