"""SE41 SentimentBot
Transforms simulated sentiment stream into governance-scored action bias.
"""
...
class SentimentBot:
    ...
    def decide(self, obs: Dict[str, Any]) -> Tuple[float, Dict[str, Any]]:
        ...
        pol = (obs["polarity"] + 1) / 2  # to [0,1]
        focus = 1.0 - min(1.0, obs["dispersion"])
        dna = [pol, focus, feats.get("confidence", 0.6), 1.0]
        score_raw = se41_numeric(M_t=pol, DNA_states=dna, harmonic_patterns=dna[::-1])
        try:
    ...
    def act(self, obs: Dict[str, Any], score: float, meta: Dict[str, Any]) -> str:
        ...
        if isinstance(dec, dict) and dec.get("decision") == "deny":
            self.state.denied += 1
            return "deny"
        if meta["polarity"] > 0.3 and meta["focus"] > 0.5:
            action = "amplify_long_bias"
        elif meta["polarity"] < -0.3 and meta["focus"] > 0.5:
            action = "amplify_short_bias"
        else:
            action = "neutral_listen"
...
def create_bot(**kwargs) -> SentimentBot:
    return SentimentBot(BotConfig(**kwargs))


def _selftest() -> int:
    r = create_bot().run_once()
    ok = 0.0 <= r["score"] <= 1.0
    print("SentimentBot selftest", "PASS" if ok else "FAIL", json.dumps(r))
