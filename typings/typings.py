"""Symbolic utilities for the `typings` data bucket."""
...
__all__ = ["TypingsSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "typings", "module": "typings"},
}


@dataclass(slots=True)
class TypingsSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> TypingsSnapshot:
    ...
    if path.exists():
        ...
        return TypingsSnapshot(bucket="typings", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = TypingsSnapshot(bucket="typings", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> TypingsSnapshot:
    ...
    snapshot = TypingsSnapshot(bucket="typings", context=ctx, signals=signals)
