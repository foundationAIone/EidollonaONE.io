"""Symbolic utilities for the `typings/ai_core/trading` data bucket."""
...
__all__ = ["TradingSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "typings/ai_core/trading", "module": "trading"},
}
...
def load_snapshot(root: Path | None = None) -> TradingSnapshot:
    ...
    if path.exists():
        ...
        return TradingSnapshot(bucket="typings/ai_core/trading", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = TradingSnapshot(bucket="typings/ai_core/trading", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> TradingSnapshot:
    ...
    snapshot = TradingSnapshot(bucket="typings/ai_core/trading", context=ctx, signals=signals)
