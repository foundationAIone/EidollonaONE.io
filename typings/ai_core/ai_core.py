"""Symbolic utilities for the `typings/ai_core` data bucket."""
...
__all__ = ["AiCoreSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "typings/ai_core", "module": "ai_core"},
}
...
def load_snapshot(root: Path | None = None) -> AiCoreSnapshot:
    ...
    if path.exists():
        ...
        return AiCoreSnapshot(bucket="typings/ai_core", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = AiCoreSnapshot(bucket="typings/ai_core", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> AiCoreSnapshot:
    ...
    snapshot = AiCoreSnapshot(bucket="typings/ai_core", context=ctx, signals=signals)
