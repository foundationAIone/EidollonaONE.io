"""Symbolic utilities for the `trade_awaken_data` data bucket."""
...
__all__ = ["TradeAwakenDataSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "trade_awaken_data", "module": "trade_awaken_data"},
}


@dataclass(slots=True)
class TradeAwakenDataSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> TradeAwakenDataSnapshot:
    ...
    if path.exists():
        ...
        return TradeAwakenDataSnapshot(bucket="trade_awaken_data", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = TradeAwakenDataSnapshot(bucket="trade_awaken_data", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> TradeAwakenDataSnapshot:
    ...
    snapshot = TradeAwakenDataSnapshot(bucket="trade_awaken_data", context=ctx, signals=signals)
