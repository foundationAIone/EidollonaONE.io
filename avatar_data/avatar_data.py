"""Symbolic utilities for the `avatar_data` data bucket."""
...
__all__ = ["AvatarDataSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "avatar_data", "module": "avatar_data"},
}


@dataclass
class AvatarDataSnapshot:
    __slots__ = ("bucket", "context", "signals")
    bucket: str
...
def load_snapshot(root: Path | None = None) -> AvatarDataSnapshot:
    ...
    if path.exists():
        ...
        return AvatarDataSnapshot(bucket="avatar_data", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = AvatarDataSnapshot(bucket="avatar_data", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> AvatarDataSnapshot:
    ...
    snapshot = AvatarDataSnapshot(bucket="avatar_data", context=ctx, signals=signals)
