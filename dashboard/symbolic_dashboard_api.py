...
_log = _logging.getLogger("dashboard.symbolic_dashboard_api.shim")


def _emit_warning_once(message: str) -> None:
    key = "_DASHBOARD_SHIM_WARNED_API"
...
_MSG = (
    "Deprecated import: 'dashboard.symbolic_dashboard_api'. "
...
def _audit_deprecated_import() -> None:
    try:
        ...
        _append_event(
            ...
            ctx={
                "module": "dashboard.symbolic_dashboard_api",
                "shim_mode": _SHIM_MODE,
            },
            payload={"module": "dashboard.symbolic_dashboard_api"},
        )
    except Exception as e:
        _log.debug("symbolic_dashboard_api shim audit skipped: %s", e)
...
try:
    ...
    raise ImportError(
        ...
        "Ensure the web_planning backend is on PYTHONPATH and that "
