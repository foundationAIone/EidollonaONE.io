...
_log = _logging.getLogger("dashboard.symbolic_dashboard.shim")


def _emit_warning_once(message: str) -> None:
    key = "_DASHBOARD_SHIM_WARNED_SYM"
...
_MSG = (
    "Deprecated import: 'dashboard.symbolic_dashboard'. "
...
def _audit_deprecated_import() -> None:
    try:
        ...
        _append_event(
            ...
            ctx={
                "module": "dashboard.symbolic_dashboard",
                "shim_mode": _SHIM_MODE,
            },
            payload={"module": "dashboard.symbolic_dashboard"},
        )
    except Exception as e:
        _log.debug("symbolic_dashboard shim audit skipped: %s", e)
