"""Symbolic utilities for the `system_backups` data bucket."""
...
__all__ = ["SystemBackupsSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "system_backups", "module": "system_backups"},
}


@dataclass(slots=True)
class SystemBackupsSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> SystemBackupsSnapshot:
    ...
    if path.exists():
        ...
        return SystemBackupsSnapshot(bucket="system_backups", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = SystemBackupsSnapshot(bucket="system_backups", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> SystemBackupsSnapshot:
    ...
    snapshot = SystemBackupsSnapshot(bucket="system_backups", context=ctx, signals=signals)
