"""Symbolic utilities for the `generated_modules` data bucket."""
...
__all__ = ["GeneratedModulesSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "generated_modules", "module": "generated_modules"},
}


@dataclass(slots=True)
class GeneratedModulesSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> GeneratedModulesSnapshot:
    ...
    if path.exists():
        ...
        return GeneratedModulesSnapshot(bucket="generated_modules", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = GeneratedModulesSnapshot(bucket="generated_modules", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> GeneratedModulesSnapshot:
    ...
    snapshot = GeneratedModulesSnapshot(bucket="generated_modules", context=ctx, signals=signals)
