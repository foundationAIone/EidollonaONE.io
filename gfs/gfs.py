"""Symbolic utilities for the `gfs` data bucket."""
...
__all__ = ["GfsSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "gfs", "module": "gfs"},
}
...
def load_snapshot(root: Path | None = None) -> GfsSnapshot:
    ...
    if path.exists():
        ...
        return GfsSnapshot(bucket="gfs", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = GfsSnapshot(bucket="gfs", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> GfsSnapshot:
    ...
    snapshot = GfsSnapshot(bucket="gfs", context=ctx, signals=signals)
