"""Symbolic utilities for the `network_diagnostics` data bucket."""
...
__all__ = ["NetworkDiagnosticsSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "network_diagnostics", "module": "network_diagnostics"},
}
...
def load_snapshot(root: Path | None = None) -> NetworkDiagnosticsSnapshot:
    ...
    if path.exists():
        ...
        return NetworkDiagnosticsSnapshot(bucket="network_diagnostics", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = NetworkDiagnosticsSnapshot(bucket="network_diagnostics", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> NetworkDiagnosticsSnapshot:
    ...
    snapshot = NetworkDiagnosticsSnapshot(bucket="network_diagnostics", context=ctx, signals=signals)
