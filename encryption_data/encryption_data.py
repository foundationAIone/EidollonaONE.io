"""Symbolic utilities for the `encryption_data` data bucket."""
...
__all__ = ["EncryptionDataSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "encryption_data", "module": "encryption_data"},
}


@dataclass(slots=True)
class EncryptionDataSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> EncryptionDataSnapshot:
    ...
    if path.exists():
        ...
        return EncryptionDataSnapshot(bucket="encryption_data", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = EncryptionDataSnapshot(bucket="encryption_data", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> EncryptionDataSnapshot:
    ...
    snapshot = EncryptionDataSnapshot(bucket="encryption_data", context=ctx, signals=signals)
