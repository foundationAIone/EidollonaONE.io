"""Symbolic utilities for the `sensing` data bucket."""
...
__all__ = ["SensingSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "sensing", "module": "sensing"},
}
...
def load_snapshot(root: Path | None = None) -> SensingSnapshot:
    ...
    if path.exists():
        ...
        return SensingSnapshot(bucket="sensing", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = SensingSnapshot(bucket="sensing", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> SensingSnapshot:
    ...
    snapshot = SensingSnapshot(bucket="sensing", context=ctx, signals=signals)
