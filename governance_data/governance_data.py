"""Symbolic utilities for the `governance_data` data bucket."""
...
__all__ = ["GovernanceDataSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "governance_data", "module": "governance_data"},
}


@dataclass(slots=True)
class GovernanceDataSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> GovernanceDataSnapshot:
    ...
    if path.exists():
        ...
        return GovernanceDataSnapshot(bucket="governance_data", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = GovernanceDataSnapshot(bucket="governance_data", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> GovernanceDataSnapshot:
    ...
    snapshot = GovernanceDataSnapshot(bucket="governance_data", context=ctx, signals=signals)
