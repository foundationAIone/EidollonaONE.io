"""Symbolic utilities for the `outputs/avatars` data bucket."""
...
__all__ = ["AvatarsSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "outputs/avatars", "module": "avatars"},
}


@dataclass(slots=True)
class AvatarsSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> AvatarsSnapshot:
    ...
    if path.exists():
        ...
        return AvatarsSnapshot(bucket="outputs/avatars", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = AvatarsSnapshot(bucket="outputs/avatars", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> AvatarsSnapshot:
    ...
    snapshot = AvatarsSnapshot(bucket="outputs/avatars", context=ctx, signals=signals)
