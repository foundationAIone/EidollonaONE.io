...
class AerBackend(Backend, ABC):
        ...
        for index, instruction in enumerate(circuit.data):
            if instruction.operation.is_parameterized():
                for bind_pos, param in enumerate(instruction.operation.params):
                    append_param_values(
                        idx_map[index] if idx_map else index, bind_pos, param
                    )
    ...
    def target(self):
        ...
        supported_instructions = set(
            getattr(configuration, "supported_instructions", [])
        )
        ...
        if properties:
            ...
            faulty_qubits = {
                ...
                if not properties.is_qubit_operational(q)
            }
    ...
    def _execute_circuits_job(
        ...
        if parameter_binds:
            ...
            raise AerError(
                "circuits have parameters but parameter_binds is not specified."
            )
