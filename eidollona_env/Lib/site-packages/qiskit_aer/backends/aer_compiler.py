...
class AerCompiler:
    ...
    def _convert_jump_conditional(self, cond_tuple, bit_map):
        ...
        if isinstance(cond_tuple, Expr):
            ...
            if isinstance(cond_tuple[0].var, Clbit):
                ...
                expr = Var(
                    [bit_map[clbit] for clbit in cond_tuple[0].var], cond_tuple[0].type
                )
            else:
                raise AerError(
    ...
    def _inline_while_loop_op(self, instruction, parent, bit_map):
        ...
        condition_tuple = self._convert_jump_conditional(
        ...
        parent.append(
            AerJump(loop_start_label, len(qargs), len(mark_cargs)).set_conditional(
                c_if_args
            ),
            qargs,
            mark_cargs,
    ...
    def _inline_if_else_op(
        ...
        parent.append(
            AerJump(if_true_label, len(qargs), len(mark_cargs)).set_conditional(
                c_if_args
            ),
            qargs,
            mark_cargs,
        ...
        child = self._inline_circuit(
            true_body, continue_label, break_label, true_bit_map
        )
    ...
    def _inline_switch_case_op(
        ...
        if isinstance(instruction.operation.target, Clbit):
            ...
            target_clbits = self._list_clbit_from_expr(
                bit_map, instruction.operation.target
            )
        else:
        ...
        CaseData = collections.namedtuple(
            "CaseData", ["label", "args_list", "bit_map", "body"]
        )
        case_data_list = []
        ...
        for case_data in case_data_list:
            for case_args in case_data.args_list:
                if len(case_args) > 0:
                    ...
                    parent.append(
                        AerJump(
                            case_data.label, len(qargs), len(mark_cargs)
                        ).set_conditional(case_args),
                        qargs,
                        mark_cargs,
                    )
        ...
        for case_data in case_data_list:
            parent.append(
                AerMark(case_data.label, len(qargs), len(mark_cargs)), qargs, mark_cargs
            )
            child = self._inline_circuit(
            ...
            parent.append(
                AerJump(switch_end_label, len(qargs), len(mark_cargs)),
                qargs,
                mark_cargs,
            )

        parent.append(
            AerMark(switch_end_label, len(qargs), len(mark_cargs)), qargs, mark_cargs
        )
...
def assemble_circuit(circuit: QuantumCircuit, basis_gates=None):
    ...
    for inst in circuit.data:
        ...
        if hasattr(inst.operation, "condition") and inst.operation.condition:
            ...
            hasattr(inst.operation, "condition_expr") and inst.operation.condition_expr
        ):
...
class _AssembleExprImpl(ExprVisitor):
    ...
    def visit_var(self, node, /):
        return AerVar(
            _assemble_type(node.type), _assemble_clbit_indices(self.circuit, node.var)
        )
    ...
    def visit_unary(self, node, /):
        return AerUnaryExpr(
            _assemble_unary_operator(node.op), node.operand.accept(self)
        )
...
def _assemble_op(
    ...
    aer_cond_expr = (
        conditional_expr.accept(_AssembleExprImpl(circ)) if conditional_expr else None
    )
...
def assemble_circuits(
    circuits: List[QuantumCircuit], basis_gates: list = None
) -> List[AerCircuit]:
    ...
    if basis_gates is not None:
        ...
        aer_circuits, idx_maps = zip(
            *[assemble_circuit(circuit) for circuit in circuits]
        )
