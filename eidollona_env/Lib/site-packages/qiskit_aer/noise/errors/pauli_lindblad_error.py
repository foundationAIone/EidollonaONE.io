...
class PauliLindbladError(BaseQuantumError, TolerancesMixin):
    ...
    def to_pauli_error(self, simplify: bool = True) -> PauliError:
        ...
        for term_z, term_x, term_r in zip(
            self.generators.z, self.generators.x, self.rates
        ):
            ...
            if simplify:
                error_op = PauliError(
                    PauliList.from_symplectic(chan_z, chan_x), chan_p
                ).simplify()
    ...
    def to_quantum_error(self) -> QuantumError:
        ...
        if not self.is_cptp():
            raise NoiseError(
                "Cannot convert non-CPTP PauliLindbladError to a QuantumError"
            )
