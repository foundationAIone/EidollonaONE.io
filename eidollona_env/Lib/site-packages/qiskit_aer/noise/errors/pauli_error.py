...
class PauliError(BaseQuantumError, TolerancesMixin):
    ...
    def tensor(self, other: PauliError) -> PauliError:
        ...
        left = SparsePauliOp(
            self.paulis, self.probabilities, copy=False, ignore_pauli_phase=True
        )
        right = SparsePauliOp(
    ...
    def compose(self, other, qargs=None, front=False) -> PauliError:
        ...
        probabilities = np.multiply.outer(
    ...
    def simplify(
        ...
        if rtol is None:
            rtol = self.rtol
        simplified = SparsePauliOp(self.paulis, self.probabilities).simplify(
            atol=atol, rtol=rtol
        )
...
def sort_paulis(
    paulis: PauliList, coeffs: Sequence | None = None
) -> tuple[PauliList, Sequence]:
    ...
    if coeffs is None:
        ...
        unsorted = (
            (*row.tolist(), coeff, i)
            for i, (row, coeff) in enumerate(zip(packed, coeffs))
        )
