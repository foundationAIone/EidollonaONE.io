...
"""
    ...
    def _initialize_latex_array(self):
        ...
        self._latex = [
            [
                "\\qw" if isinstance(wire, Qubit) else "\\cw"
            ...
            for node in layer:
                ...
                for arg in op.params:
                    if not any(isinstance(param, np.ndarray) for param in op.params):
                        arg_str = re.sub(
                            r"[-+]?\d*\.\d{2,}|\d{2,}", self._truncate_float, str(arg)
                        )
            ...
            for node in layer:
                ...
                if getattr(op, "condition", None):
                    if isinstance(op.condition, expr.Expr):
                        warn(
                            "ignoring expression condition, which is not supported yet"
                        )
                    else:
                ...
                if isinstance(op, Measure):
                    ...
                    if len(wire_list) == 1 and not node.cargs:
                        ...
                        num_cols_op = self._build_ctrl_gate(
                            op, gate_text, wire_list, column
                        )
                    else:
    ...
    def _build_symmetric_gate(self, op, gate_text, wire_list, col):
        ...
        if isinstance(op, SwapGate) or (base_op and isinstance(base_op, SwapGate)):
            ...
            self._latex[wire_last][col] = (
                "\\qswap \\qwx[" + str(wire_next_last - wire_last) + "]"
            )
            return 1  # num_cols
        ...
        self._latex[wire_max - 1][
            col + 1
        ] = f"\\dstick{{\\hspace{{2.0em}}{gate_text}}} \\qw"
    ...
    def _add_controls(self, wire_list, ctrlqargs, ctrl_state, col):
        ...
        for index, ctrl_item in enumerate(zip(ctrlqargs, ctrl_state)):
            ...
            self._latex[pos][col] = (
                f"{control}" + "{" + str(nxt - wire_list[index]) + "}"
            )
    ...
    def _add_condition(self, op, wire_list, col):
        ...
        if cond_is_bit or self._cregbundle:
            cwire = (
                self._wire_map[cond_reg]
                if self._cregbundle
                else self._wire_map[op.condition[0]]
            )
        ...
        # If condition is a register and cregbundle is false
            ...
            control = (
                "\\control"
                if val_bits_sorted[len(cond_wires) - 1] == "1"
                else "\\controlo"
            )
