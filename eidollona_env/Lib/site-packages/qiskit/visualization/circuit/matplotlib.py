...
class MatplotlibDrawer:
    ...
    def draw(self, filename=None, verbose=False):
        ...
        glob_data["subfont_factor"] = (
            self._style["sfs"] * def_font_ratio / self._style["fs"]
        )
        ...
        layer_widths = self._get_layer_widths(
            node_data, wire_map, self._circuit, glob_data
        )
        ...
        if is_user_ax:
            ...
            bbox = self._ax.get_window_extent().transformed(
                mpl_figure.dpi_scale_trans.inverted()
            )
            scale = bbox.width / base_fig_w / 0.8361111

        # if scale not 1.0, use this scale factor
        elif self._scale != 1.0:
            mpl_figure.set_size_inches(
                base_fig_w * self._scale, base_fig_h * self._scale
            )
        ...
        if self._global_phase:
            plt_mod.text(
                xl, yt, f"Global Phase: {pi_check(self._global_phase, output='mpl')}"
            )
        self._draw_regs_wires(
            num_folds, xmax, max_x_index, qubits_dict, clbits_dict, glob_data
        )
        self._draw_ops(
            self._nodes,
            node_data,
            ...
            for i, node in enumerate(layer):
                ...
                ctrl_width = (
                    self._get_text_width(
                        ctrl_text, glob_data, fontsize=self._style["sfs"]
                    )
                    - 0.05
                )
                ...
                if (
                    ...
                    and not any(
                        isinstance(param, QuantumCircuit) for param in op.params
                    )
                ):
                ...
                # Check if a ControlFlowOp - node_data load for these gates is done here
                    ...
                    if (
                        isinstance(op, SwitchCaseOp)
                        and isinstance(op.target, expr.Expr)
                    ) or (
                        getattr(op, "condition", None)
                        and isinstance(op.condition, expr.Expr)
                    ):
                        ...
                        condition = (
                            op.target if isinstance(op, SwitchCaseOp) else op.condition
                        )
                        stream = StringIO()
                    ...
                    for circ_num, circuit in enumerate(circuit_list):
                        ...
                        for outer, inner in zip(node.cargs, circuit.clbits):
                            if self._cregbundle and (
                                (in_reg := get_bit_register(outer_circuit, inner))
                                is not None
                            ):
                        ...
                        for width, layer_num, flow_parent in flow_widths.values():
                            if (
                                layer_num != -1
                                and flow_parent == flow_drawer._flow_parent
                            ):
                        ...
                        if not isinstance(op, ForLoopOp) and circ_num == 0:
                            node_data[node].width.append(
                                raw_gate_width - (expr_width % 1)
                            )
                        else:
    ...
    def _set_bit_reg_info(self, wire_map, qubits_dict, clbits_dict, glob_data):
        ...
        for ii, wire in enumerate(wire_map):
            ...
            reg_size = (
                0
                if register is None or isinstance(wire, ClassicalRegister)
                else register.size
            )
            ...
            text_width = (
                self._get_text_width(
                    ...
                    glob_data,
                    self._style["fs"],
                    reg_remove_under=reg_remove_under,
                )
                * 1.15
            )
    ...
    def _get_coords(
        ...
        for layer in self._nodes:
            ...
            for node in layer:
                ...
                if flow_parent is not None:
                    ...
                    # Add expr_width to if, while, or switch if expr used
                        ...
                        x_index = (
                            node_data[node].x_index + node_data[flow_parent].expr_width
                        )
                else:
            ...
            if not self._plot_barriers:
                ...
                barrier_offset = (
                    -1
                    if all(getattr(nd.op, "_directive", False) for nd in layer)
                    else 0
                )
    ...
    def _get_text_width(
        self, text, glob_data, fontsize, param=False, reg_remove_under=None
    ):
    ...
    def _draw_regs_wires(
        ...
        for fold_num in range(num_folds + 1):
            ...
            box = glob_data["patches_mod"].Rectangle(
                xy=(
                    glob_data["x_offset"] - 0.1,
                    -fold_num * (glob_data["n_lines"] + 1) + 0.5,
                ),
                width=-25.0,
        ...
        if self._style["index"]:
            for layer_num in range(max_x_index):
                if self._fold > 0:
                    ...
                    y_coord = (
                        -(layer_num // self._fold) * (glob_data["n_lines"] + 1) + 0.65
                    )
                else:
        ...
        for layer in nodes:
            ...
            for node in layer:
                ...
                if getattr(op, "condition", None) or isinstance(op, SwitchCaseOp):
                    ...
                    self._condition(
                        node, node_data, wire_map, outer_circuit, cond_xy, glob_data
                    )
            ...
            prev_x_index = (
                curr_x_index + (max(l_width) if l_width else 0) + barrier_offset - 1
            )
    ...
    def _condition(self, node, node_data, wire_map, outer_circuit, cond_xy, glob_data):
        ...
        if isinstance(condition, expr.Expr):
            ...
            cond_pos.extend(
                cond_xy[wire_map[bit] - first_clbit] for bit in registers.pop(None, ())
            )
            if self._cregbundle:
                cond_pos.extend(
            ...
            label, val_bits = get_condition_label_val(
                condition, self._circuit, self._cregbundle
            )
    ...
    def _flow_op_gate(self, node, node_data, glob_data):
        ...
        if_width = node_data[node].width[0] + (
            WID if not isinstance(node.op, BoxOp) else -0.19
        )
        box_width = if_width
        ...
        while end_x > 0.0:
            ...
            for circ_num, ewidth in enumerate(node_data[node].width[1:]):
                if ewidth > 0.0:
                    self._ax.plot(
                        [
                            xpos + ewidth_incr + 0.3 - x_shift,
                            xpos + ewidth_incr + 0.3 - x_shift,
                        ],
                        [
                            ypos - 0.5 * HIG - 0.08 - y_shift,
                            ypos + height - 0.22 - y_shift,
                        ],
                    ...
                    if isinstance(node.op, SwitchCaseOp):
                        ...
                        if len(str(jump_val)) == 4:
                            ...
                            # If a tuple, show first 4 elements followed by '...'
                            jump_text = (
                                str(jump_val)[1:-1].replace(" ", "").split(",")[:5]
                            )
                            if len(jump_text) > 4:
                                jump_text[4] = "..."
    ...
    def _control_gate(self, node, node_data, glob_data, mod_control):
        ...
        num_ctrl_qubits = (
            mod_control.num_ctrl_qubits if mod_control else op.num_ctrl_qubits
        )
        ...
        if isinstance(op, RZZGate) or isinstance(
            ...
            self._x_tgt_qubit(
                xy[num_ctrl_qubits], glob_data, ec=node_data[node].ec, ac=tgt
            )
    ...
    def _ctrl_qubit(
        self, xy, glob_data, fc=None, ec=None, tc=None, text="", text_top=None
    ):
