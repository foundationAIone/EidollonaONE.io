...
class TextDrawing:
    ...
    def wire_names(self, with_initial_state=False):
        ...
        for wire, index in self._wire_map.items():
            ...
            wire_label += (
                " " if self.layout is not None and isinstance(wire, Qubit) else ": "
            )
    ...
    def controlled_wires(node, wire_map, ctrl_text, conditional, mod_control):
        ...
        for i in range(len(ctrl_qubits)):
            ...
            if ctrl_state[i] == "1":
                gates.append(
                    Bullet(
                        conditional=conditional, label=ctrl_text, bottom=bool(bot_box)
                    )
                )
            else:
                gates.append(
    ...
    def _node_to_gate(self, node, layer, gate_wire_map):
        ...
        if not isinstance(op, (Measure, SwapGate, Reset)) and not getattr(
            op, "_directive", False
        ):
        ...
        if getattr(op, "condition", None) is not None:
            # conditional
            current_cons_cond += layer.set_cl_multibox(
                op.condition, gate_wire_map, top_connect="╨"
            )
            conditional = True
        ...
        if isinstance(op, Measure):
            ...
            top_qubit = min(
                node.qargs, key=lambda q: self._wire_map.get(q, float("inf"))
            )
            ...
            # unitary gate
            layer.set_qubit(
                node.qargs[0], BoxOnQuWire(gate_text, conditional=conditional)
            )
            ...
            gates, controlled_top, controlled_bot, controlled_edge, rest = (
                controls_array
            )
            if mod_control:
                if len(rest) == 1:
        ...
        for node_layer in self.nodes:
            ...
            for node in node_layer:
                if isinstance(node.op, ControlFlowOp):
                    ...
                    layer, current_cons, current_cons_cond, connection_label = (
                        self._node_to_gate(node, layer, self._wire_map)
                    )
    ...
    def add_control_flow(self, node, layers, wire_map):
        ...
        if (
            isinstance(node.op, SwitchCaseOp) and isinstance(node.op.target, expr.Expr)
        ) or (
            getattr(node.op, "condition", None)
            and isinstance(node.op.condition, expr.Expr)
        ):
            ...
            draw_conditional = isinstance(
                node.op, (IfElseOp, WhileLoopOp, SwitchCaseOp)
            )

        # # Draw a left box such as If, While, For, and Switch
        flow_layer = self.draw_flow_box(
            node, wire_map, CF_LEFT, conditional=draw_conditional
        )
        ...
        if isinstance(node.op, SwitchCaseOp):
            ...
            circuit_list.insert(
                0, list(node.op.cases_specifier())[0][1].copy_empty_like()
            )
    ...
    def draw_flow_box(
        self, node, flow_wire_map, section, circ_num=0, conditional=False
    ):
        ...
        if len(node.qargs) == 1:
            ...
            flow_layer.set_qubit(
                self.qubits[min_idx],
                FlowOnQuWireTop(section, label=label, wire_label=""),
            )
...
class Layer:
    ...
    def _set_multibox(
        ...
        if qargs is not None and cargs is not None:
            ...
            self.set_qubit(
                qargs.pop(0), BoxOnQuWireTop(label, wire_label=qargs_str.pop(0))
            )
            order = 0
            for order, bit_i in enumerate(
                range(min(qarg_indices) + 1, len(self.qubits))
            ):
            ...
            self.set_clbit(
                cargs.pop(0),
                BoxOnClWireBot(label, box_height, wire_label=cargs_str.pop(0)),
            )
            return carg_indices
        ...
        if len(bit_indices) == 1:
            ...
            set_bit(
                bits.pop(0),
                OnWireTop(label, top_connect=top_connect, wire_label=qargs_str.pop(0)),
            )
            for order, bit_i in enumerate(
                range(min(bit_indices) + 1, max(bit_indices))
            ):
    ...
    def set_cl_multibox(self, condition, wire_map, top_connect="┴"):
        ...
        if isinstance(condition, expr.Expr):
            ...
            if registerless := registers.pop(None, ()):
                out.extend(
                    self.set_cond_bullets(
                        label, ["1"] * len(registerless), registerless, wire_map
                    )
                )
            if self.cregbundle:
                ...
                for register in registers:
                    self.set_clbit(
                        register[0], BoxOnClWire(label=label, top_connect=top_connect)
                    )
            else:
                for register, bits in registers.items():
                    out.extend(
                        self.set_cond_bullets(label, ["1"] * len(bits), bits, wire_map)
                    )
            return out

        label, val_bits = get_condition_label_val(
            condition, self._circuit, self.cregbundle
        )
        ...
        if self.cregbundle:
            if isinstance(condition[0], Clbit):
                ...
                # if it's a single bit in a register
                    ...
                    self.set_clbit(
                        condition[0], BoxOnClWire(label=label, top_connect=top_connect)
                    )
            # if it's a whole register
            else:
                self.set_clbit(
                    condition[0][0], BoxOnClWire(label=label, top_connect=top_connect)
                )
            return []
        else:
            if isinstance(condition[0], Clbit):
