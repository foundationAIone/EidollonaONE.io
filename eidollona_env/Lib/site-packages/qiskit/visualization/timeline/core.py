...
class DrawerCanvas:
    ...
    def load_program(
        self, program: circuit.QuantumCircuit, target: Target | None = None
    ):
        ...
        for t0, instruction in zip(program.op_start_times, program.data):
            ...
            for bit_pos, bit in enumerate(bits):
                    ...
                    if target is not None:
                        ...
                        if op_props is not None:
                            inst_props = op_props.get(
                                tuple(
                                    program.find_bit(x).index
                                    for x in instruction.qubits
                                )
                            )
                ...
                if isinstance(instruction.operation, circuit.Barrier):
                    ...
                    barrier_source = types.Barrier(
                        t0=t0, bits=bits, bit_position=bit_pos
                    )
    ...
    def update(self):
        ...
        for bit in self.layout["bit_arrange"](self.bits):
            ...
            if (
                isinstance(bit, circuit.Clbit)
                and not self.formatter["control.show_clbits"]
            ):
                continue
            # remove idle bit
    ...
    def _check_data_visible(self, data: drawings.ElementaryData) -> bool:
        ...
        def _data_check(_data):
            ...
            if _data.data_type == str(types.LineType.GATE_LINK.value):
                active_bits = [
                    bit for bit in _data.bits if bit not in self.disable_bits
                ]
                if len(active_bits) < 2:
                    return False
            elif (
                _data.data_type in _barriers
                and not self.formatter["control.show_barriers"]
            ):
                return False
            elif (
                _data.data_type in _delays and not self.formatter["control.show_delays"]
            ):
                return False
            return True
    ...
    def _check_link_overlap(
        ...
        def y_coords(link: drawings.GateLinkData):
            return np.array(
                [self.assigned_coordinates.get(bit, np.nan) for bit in link.bits]
            )
        ...
        while len(data_keys) > 0:
            ...
            for key in data_keys[::-1]:
                ...
                if (
                    np.abs(links[ref_key].xvals[0] - links[key].xvals[0])
                    < allowed_overlap
                ):
        ...
        for overlaps in overlapped_group:
            if len(overlaps) > 1:
                ...
                sorted_keys = sorted(
                    overlaps, key=lambda x: np.nanmax(y_coords(links[x]))
                )
