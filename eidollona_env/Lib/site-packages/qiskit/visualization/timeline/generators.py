...
def gen_sched_gate(
    ...
    # find color
    color = formatter["color.gates"].get(
        gate.operand.name, formatter["color.default_gate"]
    )
...
def gen_bit_name(
    ...
    if program is None:
        warnings.warn(
            "bits cannot be accurately named without passing a 'program'", stacklevel=2
        )
        ...
        if loc.registers:
            ...
            label_latex = (
                rf"{{\rm {loc.registers[-1][0].prefix}}}_{{{loc.registers[-1][1]}}}"
            )
        else:
            label_plain = "q" if isinstance(bit, Qubit) else "c"
...
def gen_barrier(
    barrier: types.Barrier, formatter: Dict[str, Any]
...
def gen_gate_link(
    link: types.GateLink, formatter: Dict[str, Any]
