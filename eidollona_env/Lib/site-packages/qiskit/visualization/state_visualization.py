    ...
    for idx in range(eigvals.shape[0] - 1, -1, -1):
        if eigvals[idx] > 0.001:
            ...
            for i in range(2**num):
                ...
                prob = min(
                    prob, 1
                )  # See https://github.com/Qiskit/qiskit-terra/issues/4666
...
def state_to_latex(
    ...
    **args,
    ...
    is_qubit_statevector = len(operator_shape.dims_r()) == 0 and set(
        operator_shape.dims_l()
    ) == {2}
...
def _numbers_to_latex_terms(numbers: List[complex], decimals: int = 10) -> List[str]:
    ...
    for number in numbers:
        term = _num_to_latex(
            number, decimals=decimals, first_term=first_term, coefficient=True
        )
        terms.append(term)
        first_term = False
    return terms
...
def _state_to_latex_ket(
    ...
    if len(nonzero_indices) > max_size:
        nonzero_indices = (
            nonzero_indices[: max_size // 2]
            + [0]
            + nonzero_indices[-max_size // 2 + 1 :]
        )
...
class TextMatrix:
    ...
    def __init__(self, state, max_size=8, dims=None, prefix="", suffix=""):
        ...
        if dims is None:  # show dims if state is not only qubits
            if (
                isinstance(state, (Statevector, DensityMatrix))
                and set(state.dims()) == {2}
            ) or (
