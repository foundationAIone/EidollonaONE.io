...
def _plotting_core(
    ...
    labels_dict, all_pvalues, all_inds = _plot_data(
        data, labels, number_to_keep, kind=kind
    )
    rects = []
    for item, _ in enumerate(data):
        ...
        ax.set_xticklabels(
            labels_dict.keys(), rotation=70, ha="right", rotation_mode="anchor"
        )
    ...
    ax.set_ylim(
        [min_ylim, min([1.1 * sum(all_vals), max(1.1 * val for val in all_vals)])]
    )
    if "desc" in sort:
        ax.invert_xaxis()
...
def _plot_data(data, labels, number_to_keep, kind="counts"):
    ...
    if number_to_keep is not None:
        data = _unify_labels(
            _keep_largest_items(execution, number_to_keep) for execution in data
        )
