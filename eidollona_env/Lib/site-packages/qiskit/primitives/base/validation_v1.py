...
def _validate_parameter_values(
    ...
    if parameter_values is None:
        if default is None:
            raise ValueError(
                "No default `parameter_values`, optional input disallowed."
            )
    ...
    if (
        ...
        or not all(
            all(_isreal(value) for value in vector) for vector in parameter_values
        )
    ):
