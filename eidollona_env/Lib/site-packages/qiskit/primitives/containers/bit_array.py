...
class BitArray(ShapedMixin):
    ...
    def __init__(self, array: NDArray[np.uint8], num_bits: int):
        ...
        if array.shape[-1] != (expected := _min_num_bytes(num_bits)):
            raise ValueError(
                f"The input array is expected to have {expected} bytes per shot."
            )
    ...
    def _prepare_broadcastable(
        ...
        try:
            shape = np.broadcast_shapes(self_shape, other_shape) + (
                self._array.shape[-1],
            )
        except ValueError as ex:
            raise ValueError(
                f"{self} and {other} are not compatible for this operation."
            ) from ex
        return np.broadcast_to(self.array, shape), np.broadcast_to(other.array, shape)

    def __and__(self, other: "BitArray") -> "BitArray":
        return BitArray(
            np.bitwise_and(*self._prepare_broadcastable(other)), self.num_bits
        )
    ...
    def __or__(self, other: "BitArray") -> "BitArray":
        return BitArray(
            np.bitwise_or(*self._prepare_broadcastable(other)), self.num_bits
        )

    def __xor__(self, other: "BitArray") -> "BitArray":
        return BitArray(
            np.bitwise_xor(*self._prepare_broadcastable(other)), self.num_bits
        )
    ...
    def _get_counts(
        self,
        *,
        loc: int | tuple[int, ...] | None,
        converter: Callable[[bytes], str | int],
    ) -> dict[str, int] | dict[int, int]:
        arr = (
            self._array.reshape(-1, self._array.shape[-1])
            if loc is None
            else self._array[loc]
        )
    ...
    def from_counts(
        ...
        counts = [
            mapping.int_outcomes() if isinstance(mapping, Counts) else mapping
        ...
        data = (
            v
            for mapping in counts
            for vs, count in mapping.items()
            for v in repeat(vs, count)
        )
        ...
        if not singleton:
            if bit_array.num_shots % len(counts) > 0:
                raise ValueError(
                    "All of your mappings need to have the same number of shots."
                )
            bit_array = bit_array.reshape(
                len(counts), bit_array.num_shots // len(counts)
            )
        return bit_array
    ...
    def to_bool_array(
        self, order: Literal["big", "little"] = "big"
    ) -> NDArray[np.bool_]:
    ...
    def postselect(
        ...
        if (
            np.intersect1d(indices[selection], indices[np.logical_not(selection)]).size
            > 0
        ):
            return BitArray(
                np.empty((0, num_bytes), dtype=np.uint8), num_bits=self.num_bits
            )
        ...
        np.bitwise_or.at(
            selection_bytes,
            byte_idx,
            np.asarray(selection, dtype=np.uint8) << bit_offset,
        )

        return BitArray(
            flattened._array[
                ((flattened._array & bitmask) == selection_bytes).all(axis=-1)
            ],
    ...
    def expectation_values(
        ...
        arr_indices = np.fromiter(np.ndindex(self.shape), dtype=object).reshape(
            self.shape
        )
    ...
    def concatenate(bit_arrays: Sequence[BitArray], axis: int = 0) -> BitArray:
        ...
        if axis < 0 or axis >= ndim:
            raise ValueError(
                f"axis {axis} is out of bounds for bit array of dimension {ndim}."
            )
