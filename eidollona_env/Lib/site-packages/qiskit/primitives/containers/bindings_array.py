...
class BindingsArray(ShapedMixin):
    ...
    def __repr__(self):
        ...
        if self.num_parameters:
            names = list(
                islice(map(repr, chain.from_iterable(map(_format_key, self._data))), 5)
            )
    ...
    def as_array(self, parameters: Iterable[ParameterLike] | None = None) -> np.ndarray:
        ...
        if parameters is None:
            ...
            idx_lookup = {
                _param_name(parameter): idx for idx, parameter in enumerate(parameters)
            }
            for arr_params, arr in self.data.items():
                try:
                    idxs = [
                        idx_lookup[_param_name(param)] + position
                        for param in arr_params
                    ]
                except KeyError as ex:
                    missing = next(
                        p for p in map(_param_name, arr_params) if p not in idx_lookup
                    )
                    raise ValueError(
                        f"Could not find placement for parameter '{missing}'."
                    ) from ex
                ret[..., idxs] = arr
    ...
    def reshape(self, *shape: int | Iterable[int]) -> BindingsArray:
        ...
        data = {
            ps: val.reshape(shape + val.shape[-1:]) for ps, val in self._data.items()
        }
