        ...
        for i, experiment_result in enumerate(result.results):
            ...
            if indices is not None:
                ...
                if csize is not None:
                    experiment_result.header["creg_sizes"] = _adjust_creg_sizes(
                        csize, indices
                    )

            if (
                getattr(experiment_result.data, "memory", None) is not None
...
def marginal_memory(
    ...
    if int_return and hex_return:
        raise ValueError(
            "Either int_return or hex_return can be specified but not both"
        )
...
def marginal_distribution(
    ...
    if indices is not None and (
        len(indices) == 0 or not set(indices).issubset(range(num_clbits))
    ):
...
def _format_marginal(counts, marg_counts, indices):
    ...
    for count in marg_counts:
        ...
        count_bits = "".join(
            [
                index_dict[index] if index in index_dict else "_"
