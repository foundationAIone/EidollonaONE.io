    ...
    for symbol, value in obj._parameter_symbols.items():
        ...
        if value == symbol._symbol_expr:
            ...
            value_key, value_data = dumps_value(
                value, version=version, use_symengine=use_symengine
            )
...
class _ExprWriter(expr.ExprVisitor[None]):
    ...
    def visit_value(self, node, /):
        ...
        if node.value is True or node.value is False:
            ...
            self.file_obj.write(
                struct.pack(
                    formats.EXPR_VALUE_BOOL_PACK, *formats.EXPR_VALUE_BOOL(node.value)
                )
            )
            ...
            self.file_obj.write(
                struct.pack(
                    formats.EXPR_VALUE_INT_PACK, *formats.EXPR_VALUE_INT(num_bytes)
                )
            )
            ...
            self.file_obj.write(
                struct.pack(
                    formats.EXPR_VALUE_FLOAT_PACK, *formats.EXPR_VALUE_FLOAT(node.value)
                )
            )
    ...
    def visit_cast(self, node, /):
        ...
        self.file_obj.write(
            struct.pack(
                formats.EXPRESSION_CAST_PACK, *formats.EXPRESSION_CAST(node.implicit)
            )
        )
    ...
    def visit_unary(self, node, /):
        ...
        self.file_obj.write(
            struct.pack(
                formats.EXPRESSION_UNARY_PACK, *formats.EXPRESSION_UNARY(node.op.value)
            )
        )
        node.operand.accept(self)
    ...
    def visit_binary(self, node, /):
        ...
        self.file_obj.write(
            struct.pack(
                ...
                *formats.EXPRESSION_BINARY(node.op.value),
            )
        )
...
def _write_duration(file_obj, duration: Duration):
    ...
    if unit == "dt":
        ...
        file_obj.write(
            struct.pack(
                formats.DURATION_DT_PACK, *formats.DURATION_DT(duration.value())
            )
        )
    elif unit == "ns":
        ...
        file_obj.write(
            struct.pack(
                formats.DURATION_NS_PACK, *formats.DURATION_NS(duration.value())
            )
        )
    elif unit == "us":
        ...
        file_obj.write(
            struct.pack(
                formats.DURATION_US_PACK, *formats.DURATION_US(duration.value())
            )
        )
    elif unit == "ms":
        ...
        file_obj.write(
            struct.pack(
                formats.DURATION_MS_PACK, *formats.DURATION_MS(duration.value())
            )
        )
    elif unit == "s":
        file_obj.write(type_keys.CircuitDuration.S)
        file_obj.write(
            struct.pack(formats.DURATION_S_PACK, *formats.DURATION_S(duration.value()))
        )
    else:
    ...
    for _ in range(data.map_elements):
        ...
        if elem_key == type_keys.Value.INTEGER:
            ...
            raise exceptions.QpyError(
                f"Invalid parameter expression map type: {elem_key}"
            )
...
def _read_parameter_expr_v13(buf, symbol_map, version, vectors):
    param_uuid_map = {
        symbol.uuid: symbol for symbol in symbol_map if isinstance(symbol, Parameter)
    }
...
def _read_expr(
    ...
    if type_key == type_keys.Expression.VAR:
        ...
        if var_type_key == type_keys.ExprVar.UUID:
            payload = formats.EXPR_VAR_UUID._make(
                struct.unpack(
                    formats.EXPR_VAR_UUID_PACK,
                    file_obj.read(formats.EXPR_VAR_UUID_SIZE),
                )
            )
        ...
        if var_type_key == type_keys.ExprVar.CLBIT:
            payload = formats.EXPR_VAR_CLBIT._make(
                struct.unpack(
                    formats.EXPR_VAR_CLBIT_PACK,
                    file_obj.read(formats.EXPR_VAR_CLBIT_SIZE),
                )
            )
        ...
        if var_type_key == type_keys.ExprVar.REGISTER:
            payload = formats.EXPR_VAR_REGISTER._make(
                struct.unpack(
                    formats.EXPR_VAR_REGISTER_PACK,
                    file_obj.read(formats.EXPR_VAR_REGISTER_SIZE),
                )
            )
    ...
    if type_key == type_keys.Expression.STRETCH:
        payload = formats.EXPRESSION_STRETCH._make(
            struct.unpack(
                formats.EXPRESSION_STRETCH_PACK,
                file_obj.read(formats.EXPRESSION_STRETCH_SIZE),
            )
        )
        return standalone_vars[payload.var_index]
    if type_key == type_keys.Expression.VALUE:
        ...
        if value_type_key == type_keys.ExprValue.BOOL:
            payload = formats.EXPR_VALUE_BOOL._make(
                struct.unpack(
                    formats.EXPR_VALUE_BOOL_PACK,
                    file_obj.read(formats.EXPR_VALUE_BOOL_SIZE),
                )
            )
        ...
        if value_type_key == type_keys.ExprValue.INT:
            payload = formats.EXPR_VALUE_INT._make(
                struct.unpack(
                    formats.EXPR_VALUE_INT_PACK,
                    file_obj.read(formats.EXPR_VALUE_INT_SIZE),
                )
            )
            return expr.Value(
        ...
        if value_type_key == type_keys.ExprValue.FLOAT:
            payload = formats.EXPR_VALUE_FLOAT._make(
                struct.unpack(
                    formats.EXPR_VALUE_FLOAT_PACK,
                    file_obj.read(formats.EXPR_VALUE_FLOAT_SIZE),
                )
            )
            return expr.Value(payload.value, type_)
...
def _read_duration(file_obj) -> Duration:
    ...
    if type_key == type_keys.CircuitDuration.DT:
        elem = formats.DURATION_DT._make(
            struct.unpack(
                formats.DURATION_DT_PACK, file_obj.read(formats.DURATION_DT_SIZE)
            )
        )
    ...
    if type_key == type_keys.CircuitDuration.NS:
        elem = formats.DURATION_NS._make(
            struct.unpack(
                formats.DURATION_NS_PACK, file_obj.read(formats.DURATION_NS_SIZE)
            )
        )
    ...
    if type_key == type_keys.CircuitDuration.US:
        elem = formats.DURATION_US._make(
            struct.unpack(
                formats.DURATION_US_PACK, file_obj.read(formats.DURATION_US_SIZE)
            )
        )
    ...
    if type_key == type_keys.CircuitDuration.MS:
        elem = formats.DURATION_MS._make(
            struct.unpack(
                formats.DURATION_MS_PACK, file_obj.read(formats.DURATION_MS_SIZE)
            )
        )
    ...
    if type_key == type_keys.CircuitDuration.S:
        elem = formats.DURATION_S._make(
            struct.unpack(
                formats.DURATION_S_PACK, file_obj.read(formats.DURATION_S_SIZE)
            )
        )
...
def write_standalone_vars(file_obj, circuit, version):
    ...
    for var in circuit.iter_input_vars():
        _write_standalone_var(
            file_obj, var, type_keys.ExprVarDeclaration.INPUT, version
        )
        out[var] = index
        index += 1
    for var in circuit.iter_captured_vars():
        _write_standalone_var(
            file_obj, var, type_keys.ExprVarDeclaration.CAPTURE, version
        )
        out[var] = index
        index += 1
    ...
    for var in circuit.iter_captured_stretches():
        _write_standalone_var(
            file_obj, var, type_keys.ExprVarDeclaration.STRETCH_CAPTURE, version
        )
        out[var] = index
        index += 1
    for var in circuit.iter_declared_stretches():
        _write_standalone_var(
            file_obj, var, type_keys.ExprVarDeclaration.STRETCH_LOCAL, version
        )
        out[var] = index
        index += 1
    return out
    ...
    *,
    ...
    if type_key == type_keys.Value.INTEGER:
        ...
        standalone_var_indices = (
            {} if standalone_var_indices is None else standalone_var_indices
        )
        binary_data = common.data_to_binary(
            obj,
            _write_expr,
