    ...
    def serialize(self, annotation: Annotation) -> (int, bytes):
        ...
        for namespace in iter_namespaces(annotation.namespace):
            if (existing := self.serializers.get(namespace, None)) is not None:
                ...
                if (
                    out := serializer.dump_annotation(namespace, annotation)
                ) is not NotImplemented:
                    return index, out
            if (
                serializer := self.potential_serializers.get(namespace, None)
            ) is not None:
                if (
                    out := serializer.dump_annotation(namespace, annotation)
                ) is not NotImplemented:
    ...
    def initialize(self, namespace: str, payload: bytes):
        ...
        for parent_namespace in iter_namespaces(namespace):
            if (factory := self.factories.get(parent_namespace, None)) is not None:
                ...
                return
        raise QpyError(
            f"No configured annotation deserializer matched namespace '{namespace}'"
        )
...
def _read_header_v12(file_obj, version, vectors, metadata_deserializer=None):
    data = formats.CIRCUIT_HEADER_V12._make(
        struct.unpack(
            formats.CIRCUIT_HEADER_V12_PACK,
            file_obj.read(formats.CIRCUIT_HEADER_V12_SIZE),
        )
    )
...
def _read_annotation_states(
    file_obj, annotation_factories
) -> _AnnotationDeserializationState:
    ...
    for _ in range(static_payload.num_namespaces):
        ...
        state.initialize(
            file_obj.read(payload.namespace_size).decode("utf-8"),
            file_obj.read(payload.state_size),
        )
    return state
...
def _read_instruction_annotation(file_obj, annotation_state):
    ...
    return annotation_state.load(
        header.namespace_index, file_obj.read(header.payload_size)
    )
...
def _read_instruction(
    ...
    if version < 5:
        ...
        conditional_key = (
            type_keys.Condition.TWO_TUPLE
            if instruction.has_condition
            else type_keys.Condition.NONE
        )
    ...
    # Load annotations.
    annotations = (
        _read_instruction_annotations(file_obj, annotation_state)
        if has_annotations
        else None
    )
...
def _write_instruction(
    ...
    if annotations:
        ...
        file_obj.write(
            struct.pack(formats.INSTRUCTION_ANNOTATIONS_HEADER_PACK, len(annotations))
        )
    ...
    *,
    ...
    if type_key == type_keys.CircuitInstruction.PAULI_EVOL_GATE:
        ...
        data = common.data_to_binary(
            operation, _write_pauli_evolution_gate, version=version
        )
        size = len(data)
...
def _write_registers(file_obj, in_circ_regs, full_bits):
    ...
    for regs, is_in_circuit in [(in_circ_regs, True), (out_circ_regs, False)]:
        for reg in regs:
            standalone = all(
                bit._register == reg and bit._index == index
...
def write_circuit(
    ...
    global_phase_type, global_phase_data = value.dumps_value(
        circuit.global_phase, version=version
    )
    ...
    if version >= 15:
        file_obj.write(
            struct.pack(
                formats.ANNOTATION_HEADER_STATIC_PACK, annotation_state.num_serializers
            )
        )
...
def read_circuit(
    ...
    var_segments, standalone_var_indices = value.read_standalone_vars(
        file_obj, num_identifiers
    )
    circ = QuantumCircuit(
        out_bits["q"],
        out_bits["c"],
    ...
    custom_operations = _read_custom_operations(
        file_obj, version, vectors, annotation_state
    )
