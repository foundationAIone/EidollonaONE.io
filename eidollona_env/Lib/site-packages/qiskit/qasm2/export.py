...
def dumps(circuit: QuantumCircuit, /) -> str:
    ...
    regless_qubits = [
        bit for bit in circuit.qubits if not circuit.find_bit(bit).registers
    ]
    regless_clbits = [
        bit for bit in circuit.clbits if not circuit.find_bit(bit).registers
    ]
    ...
    for instruction in circuit._data:
        ...
        if operation.name == "measure":
            ...
            instruction_qasm = _custom_operation_statement(
                instruction, gates_to_define, bit_labels
            )
...
def _make_unique(name: str, already_defined: collections.abc.Set[str]) -> str:
    ...
    if name not in already_defined:
        return name
    used = {
        in_use[len(name) :] for in_use in already_defined if in_use.startswith(name)
    }
    characters = (
        (string.digits + string.ascii_letters) if name else string.ascii_letters
    )
...
def _define_custom_operation(operation, gates_to_define):
    ...
    if operation.base_class in known_good_parameterized:
        parameterized_operation = type(operation)(
            *_FIXED_PARAMETERS[: len(operation.params)]
        )
        ...
        parameterized_operation = operation = new_op.copy(
            name=_escape_name(new_op.name, "gate_")
        )
    else:
    ...
    if parameterized_definition is None:
        ...
        qubit_labels = {
            bit: f"q{i}" for i, bit in enumerate(parameterized_definition.qubits)
        }
        body_qasm = " ".join(
        ...
        definition_qasm = (
            f"gate {new_name}{parameters_qasm} {qubits_qasm} {{ {body_qasm} }}"
        )
