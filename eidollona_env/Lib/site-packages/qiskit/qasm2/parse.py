...
def _generate_delay(time: float):
    ...
    if int(time) != time:
        raise QASM2ParseError(
            "the custom 'delay' instruction can only accept an integer parameter"
        )
...
class _U0Gate(Gate):
    def __init__(self, count):
        if int(count) != count:
            raise QASM2ParseError(
                "the number of single-qubit delay lengths must be an integer"
            )
...
def from_bytecode(bytecode, custom_instructions: Iterable[CustomInstruction]):
    ...
    for op in bc:
        ...
        if opcode == OpCode.Gate:
            ...
            qc._append(
                CircuitInstruction(
                    gates[gate_id](*parameters), [qubits[q] for q in op_qubits]
                )
            )
            ...
            qc._append(
                CircuitInstruction(Measure(), (qubits[qubit],), (clbits[clbit],))
            )
            ...
            qc._append(
                CircuitInstruction(
                    Barrier(len(op_qubits)), [qubits[q] for q in op_qubits]
                )
            )
        ...
        for op in self._bytecode:
            if op.opcode == OpCode.Gate:
                ...
                qc._append(
                    CircuitInstruction(
                        self._gates[gate_id](
                            *(_evaluate_argument(a, self.params) for a in args)
                        ),
...
def _evaluate_argument(expr, parameters):  # pylint: disable=too-many-return-statements
    ...
    if isinstance(expr, ExprCustom):
        return expr.callable(
            *(_evaluate_argument(x, parameters) for x in expr.arguments)
        )
