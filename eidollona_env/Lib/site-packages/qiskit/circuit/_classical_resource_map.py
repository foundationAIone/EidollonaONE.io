...
class VariableMapper(expr.ExprVisitor[expr.Expr]):
    ...
    def __init__(
        ...
        var_map: (
            typing.Mapping[expr.Var | expr.Stretch, expr.Var | expr.Stretch] | None
        ) = None,
        *,
    ...
    def map_condition(self, condition, /, *, allow_reorder=False):
        ...
        value_bits = f"{value:0{len(target)}b}"[
            ::-1
        ]  # Little-index-indexed binary bitstring.
        mapped_value = int(
            "".join(value_bits[new_order[bit]] for bit in mapped_theirs)[::-1], 2
        )
    ...
    def visit_binary(self, node, /):
        return expr.Binary(
            node.op, node.left.accept(self), node.right.accept(self), node.type
        )
