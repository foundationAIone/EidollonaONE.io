...
def _ctrl_state_to_int(ctrl_state, num_ctrl_qubits):
    ...
    if isinstance(ctrl_state, str):
        try:
            ...
            raise CircuitError(
                "invalid control bit string: length != num_ctrl_qubits"
            ) from ex
    if isinstance(ctrl_state, int):
...
def with_gate_array(base_array):
    ...
    def decorator(cls):
        if hasattr(cls, "__array__"):
            raise RuntimeError(
                "Refusing to decorate a class that already has '__array__' defined."
            )
...
def with_controlled_gate_array(base_array, num_ctrl_qubits, cached_states=None):
    ...
    if cached_states is None:
        nonwritables = [
            matrix_for_control_state(state) for state in range(2**num_ctrl_qubits)
        ]
        ...
        nonwritables = {
            state: matrix_for_control_state(state) for state in cached_states
        }
        ...
        def __array__(self, dtype=None, copy=_numpy_compat.COPY_ONLY_IF_NEEDED):
            ...
            return numpy.asarray(
                _compute_control_matrix(base, num_ctrl_qubits, self.ctrl_state),
                dtype=dtype,
            )
