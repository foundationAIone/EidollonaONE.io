    ...
    def validate_parameter(self, parameter):
        ...
        if isinstance(parameter, int):
            ...
            if parameter.type.kind is not types.Duration:
                raise CircuitError(
                    f"Expression duration of type '{parameter.type}' is not valid."
                )
            ...
            if not parameter.is_real():
                raise CircuitError(
                    f"Bound parameter expression is complex in delay {self.name}"
                )
            fval = float(parameter)
            if fval < 0:
                raise CircuitError(
                    f"Duration for Delay instruction must be positive. Found {fval}"
                )
            if self.unit == "dt":
                ival = int(parameter)
                ...
                if rounding_error > 1e-15:
                    raise CircuitError(
                        "Integer parameter is required for duration in 'dt' unit."
                    )
                return ival
            return fval  # per default assume parameters must be real when bound
        else:
            raise CircuitError(
                f"Invalid param type {type(parameter)} for delay {self.name}."
            )
