...
def _handle_equal_types(
    lvalue: expr.Expr, rvalue: expr.Expr, /
) -> tuple[expr.Expr, expr.Expr]:
    return lvalue, rvalue


def _handle_implicit_cast(
    lvalue: expr.Expr, rvalue: expr.Expr, /
) -> tuple[expr.Expr, expr.Expr]:
...
def _requires_lossless_cast(lvalue: expr.Expr, rvalue: expr.Expr, /) -> typing.NoReturn:
    raise CircuitError(
        f"an explicit cast is required from '{rvalue.type}' to '{lvalue.type}'"
    )


def _requires_dangerous_cast(
    lvalue: expr.Expr, rvalue: expr.Expr, /
) -> typing.NoReturn:
    raise CircuitError(
