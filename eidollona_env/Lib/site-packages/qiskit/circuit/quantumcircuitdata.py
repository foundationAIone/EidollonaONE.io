...
class QuantumCircuitData(MutableSequence):
    ...
    def _resolve_legacy_value(self, operation, qargs, cargs) -> CircuitInstruction:
        ...
        if not isinstance(operation, Operation) and hasattr(
        ...
        expanded_qargs = [
            self._circuit._qbit_argument_conversion(qarg) for qarg in qargs or []
        ]
        expanded_cargs = [
            self._circuit._cbit_argument_conversion(carg) for carg in cargs or []
        ]
