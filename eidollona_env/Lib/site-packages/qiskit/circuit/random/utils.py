...
def random_circuit_from_graph(
    ...
    while edges_used:
        ...
        if conditional:
            ...
            cond_val_2q = rng.integers(
                0, 1 << min(num_qubits, 63), size=np.count_nonzero(cond_2q)
            )
            clbit_2q_idx = 0
        ...
        if insert_1q_oper:
            ...
            if not num_unused_qubits == 0:
                ...
                extra_1q_gates = rng.choice(
                    gates_1q, size=num_unused_qubits, replace=True
                )
...
def random_circuit(
    ...
    if num_operand_distribution:
        if (
            min(num_operand_distribution.keys()) < 1
            or max(num_operand_distribution.keys()) > 4
        ):
            raise CircuitError(
                "'num_operand_distribution' must have keys between 1 and 4"
            )
    ...
    if not np.isclose(sum(num_operand_distribution.values()), 1):
        raise CircuitError(
            "The sum of all the values in 'num_operand_distribution' is not 1."
        )
            ...
            for key, dist in sorted(num_operand_distribution.items(), reverse=True):
                if slack >= key and counter[key] / total_gates < dist:
                    gate_to_add = np.array(
                        all_gate_lists[key - 1][
                            rng.integers(0, len(all_gate_lists[key - 1]))
                        ]
                    )
