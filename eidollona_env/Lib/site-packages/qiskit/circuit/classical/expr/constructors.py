...
def lift_legacy_condition(
    ...
    if isinstance(target, Clbit):
        ...
        return (
            Var(target, bool_)
            if value
            else Unary(Unary.Op.LOGIC_NOT, Var(target, bool_), bool_)
        )
...
def _equal_like(op: Binary.Op, left: typing.Any, right: typing.Any) -> Expr:
    ...
    return Binary(
        op, _coerce_lossless(left, type), _coerce_lossless(right, type), types.Bool()
    )
    ...
    if isinstance(left, Expr):
        if type is not None:
            ...
            if coerced_left is None:
                raise TypeError(
                    f"type '{type}' cannot losslessly represent '{left.type}'"
                )
            left = coerced_left
    else:
        left = lift(left, type)
...
def shift_left(
    left: typing.Any, right: typing.Any, /, type: types.Type | None = None
) -> Expr:
...
def index(target: typing.Any, index: typing.Any, /) -> Expr:
    ...
    if target.type.kind is not types.Uint or index.type.kind is not types.Uint:
        raise TypeError(
            f"invalid types for indexing: '{target.type}' and '{index.type}'"
        )
...
Duration())
    ...
    if left.type.kind is right.type.kind and left.type.kind in {
    ...
    elif left.type.kind is types.Duration and right.type.kind in {
        types.Uint,
        types.Float,
    }:
        type = left.type
    elif right.type.kind is types.Duration and left.type.kind in {
        types.Uint,
        types.Float,
    }:
        type = right.type
        ...
        raise TypeError(
            f"invalid types for '{Binary.Op.MUL}': '{left.type}' and '{right.type}'"
        )
    return Binary(
        Binary.Op.MUL,
        left,
...
Duration())
    ...
    if left.type.kind is right.type.kind and left.type.kind in {
        ...
        raise TypeError(
            f"invalid types for '{Binary.Op.DIV}': '{left.type}' and '{right.type}'"
        )
    return Binary(
        Binary.Op.DIV,
        left,
