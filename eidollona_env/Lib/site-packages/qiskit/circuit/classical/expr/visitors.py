...
class ExprVisitor(typing.Generic[_T_co]):
    ...
    def visit_generic(self, node: expr.Expr, /) -> _T_co:  # pragma: no cover
        raise RuntimeError(
            f"expression visitor {self} has no method to handle expr {node}"
        )
...
class _IdentWalkerImpl(
    ExprVisitor[typing.Iterable[typing.Union[expr.Var, expr.Stretch]]]
):
    __slots__ = ()
...
def iter_identifiers(
    node: expr.Expr,
) -> typing.Iterator[typing.Union[expr.Var, expr.Stretch]]:
...
class _StructuralEquivalenceImpl(ExprVisitor[bool]):
    ...
    def visit_var(self, node, /):
        ...
        if self.self_key is None or (self_var := self.self_key(node.var)) is None:
            self_var = node.var
        if (
            self.other_key is None
            or (other_var := self.other_key(self.other.var)) is None
        ):
