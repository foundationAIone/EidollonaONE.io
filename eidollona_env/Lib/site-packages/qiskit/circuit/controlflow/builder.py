...
class ControlFlowBuilderBlock(CircuitScopeInterface):
    ...
    def add_stretch(self, stretch: expr.Stretch):
        ...
        if (
            stretch.name in self._vars_capture
            or stretch.name in self._stretches_capture
        ):
            raise CircuitError(
                f"cannot add '{stretch}' as its name shadows the existing '{previous}'"
            )
    ...
    def remove_var(self, var: expr.Var):
        if self._built:
            raise RuntimeError(
                "exception handler 'remove_var' called after scope built"
            )
    ...
    def remove_stretch(self, stretch: expr.Stretch):
        if self._built:
            raise RuntimeError(
                "exception handler 'remove_stretch' called after scope built"
            )
    ...
    def use_var(self, var: expr.Var):
        if (local := self._stretches_local.get(var.name)) is not None:
            raise CircuitError(
                f"cannot use '{var}' which is shadowed by the local '{local}'"
            )
    ...
    def use_stretch(self, stretch: expr.Stretch):
        if (local := self._vars_local.get(stretch.name)) is not None:
            raise CircuitError(
                f"cannot use '{stretch}' which is shadowed by the local '{local}'"
            )
    ...
    def add_bits(self, bits: Iterable[Union[Qubit, Clbit]]):
        ...
        for bit in bits:
            if isinstance(bit, Qubit):
                ...
                raise TypeError(
                    f"Can only add qubits or classical bits, but received '{bit}'."
                )
    ...
    def build(
        ...
        if self._forbidden_message is not None:
            ...
            raise RuntimeError(
                "Cannot build a forbidden scope. Please report this as a bug."
            )
