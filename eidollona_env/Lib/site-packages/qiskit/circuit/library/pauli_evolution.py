...
class PauliEvolutionGate(Gate):
    ...
    **References:**
    ...
    def __init__(
        ...
        num_qubits = (
            operator[0].num_qubits
            if isinstance(operator, list)
            else operator.num_qubits
        )
        super().__init__(
            name="PauliEvolution", num_qubits=num_qubits, params=[time], label=label
        )
...
def _to_sparse_op(
    ...
    if any(isinstance(coeff, ParameterExpression) for coeff in sparse.coeffs):
        raise ValueError(
            "Operator contains ParameterExpression, which are not supported."
        )
