...
class PiecewiseLinearPauliRotations(FunctionalPauliRotations):
    ...
    def evaluate(self, x: float) -> float:
        ...
        y = (x >= self.breakpoints[0]) * (
            x * self.mapped_slopes[0] + self.mapped_offsets[0]
        )
    ...
    def _check_configuration(self, raise_on_failure: bool = True) -> bool:
        ...
        if len(self.breakpoints) != len(self.slopes) or len(self.breakpoints) != len(
            self.offsets
        ):
            valid = False
            if raise_on_failure:
                raise ValueError(
                    "Mismatching sizes of breakpoints, slopes and offsets."
                )
            ...
            if i == 0 and self.contains_zero_breakpoint:
                ...
                with warnings.catch_warnings():
                    ...
                    comp = IntegerComparator(
                        num_state_qubits=self.num_state_qubits, value=point
                    )
                ...
                circuit.append(
                    lin_r.to_gate().control(), qr_compare[:] + qr_state[:] + qr_target
                )
...
class PiecewiseLinearPauliRotationsGate(Gate):
    ...
    def __init__(
        ...
        super().__init__(
            "PwLinPauliRot", num_state_qubits + 1 + num_compare_bits, [], label=label
        )
    ...
    def _define(self):
        ...
        for i, point in enumerate(self.breakpoints):
            if i == 0 and contains_zero_breakpoint:
                ...
                # apply Comparator
                comp = IntegerComparatorGate(
                    num_state_qubits=num_state_qubits, value=point
                )
                qr = qr_state[:] + qr_compare[:]  # add ancilla as compare qubit
