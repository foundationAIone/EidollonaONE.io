...
class QuadraticForm(QuantumCircuit):
    ...
    def __init__(
        ...
        linear: Optional[
            Union[np.ndarray, List[Union[float, ParameterExpression]]]
        ] = None,
            ...
            for j in range(num_input_qubits):
                for k in range(j + 1, num_input_qubits):
                    ...
                    if value != 0:
                        for i, q_i in enumerate(qr_result):
                            circuit.mcp(
                                scaling * 2**i * value, [qr_input[j], qr_input[k]], q_i
                            )
...
class QuadraticFormGate(Gate):
    ...
    def required_result_qubits(
        ...
        for condition in [lambda x: x < 0, lambda x: x > 0]:
            bound = 0.0
            bound += sum(
                sum(q_ij for q_ij in q_i if condition(q_ij)) for q_i in quadratic
            )
