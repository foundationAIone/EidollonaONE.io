...
class LinearAmplitudeFunction(QuantumCircuit):
    ...
    def __init__(
        ...
        for i in range(len(breakpoints)):
            ...
            offset_angles[i] += (
                np.pi * rescaling_factor * (mapped_offset[i] - c) / 2 / (d - c)
            )
...
class LinearAmplitudeFunctionGate(Gate):
    ...
    def __init__(
        ...
        super().__init__(
            "LinFunction", num_state_qubits + num_compare + 1, [], label=label
        )
    ...
    def _define(self):
        ...
        offset_angles = (
            np.pi / 4 * (1 - self.rescaling_factor) * np.ones_like(slope_angles)
        )
        for i, (slope_i, offset_i) in enumerate(zip(mapped_slope, mapped_offset)):
            ...
            offset_angles[i] += (
                np.pi * self.rescaling_factor * (offset_i - c) / 2 / (d - c)
            )
