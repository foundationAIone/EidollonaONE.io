...
class NLocal(BlueprintCircuit):
    ...
    def __init__(
        ...
        self._allow_fast_path_parametrization = (
            getattr(self._parameter_generator, "__func__", None)
            is NLocal._parameter_generator
        )
    ...
    def rotation_blocks(
        self,
        blocks: QuantumCircuit | list[QuantumCircuit] | Instruction | list[Instruction],
    ) -> None:
    ...
    def get_unentangled_qubits(self) -> set[int]:
        ...
        for i in range(self._reps):
            for j, block in enumerate(self.entanglement_blocks):
                ...
                entangled_qubits.update(
                    [idx for indices in entangler_map for idx in indices]
                )
    ...
    def get_entangler_map(
        ...
        if all(isinstance(en, str) for en in entanglement):
            return get_entangler_map(
                n, self.num_qubits, entanglement[i % num_i], offset=i
            )
        ...
        if all(
            isinstance(e2, (int, numpy.int32, numpy.int64))
    ...
    def assign_parameters(
        self,
        parameters: (
            Mapping[Parameter, ParameterExpression | float]
    ...
    def _build_rotation_layer(self, circuit, param_iter, i):
        ...
        for j, block in enumerate(self.rotation_blocks):
            ...
            if (
                ...
                all_qubits = (
                    tuple(
                        target_qubits[k * block.num_qubits : (k + 1) * block.num_qubits]
                    )
                ...
                for qubits in all_qubits:
                    instr = CircuitInstruction(
                        simple_block.gate(
                            *itertools.islice(param_iter, simple_block.num_params)
                        ),
                        qubits,
                    )
                ...
                for indices in block_indices:
                    ...
                    circuit.compose(
                        parameterized_block, indices, inplace=True, copy=False
                    )
...
def get_entangler_map(
    ...
    """
    try:
        return fast_entangler_map(
            num_circuit_qubits, num_block_qubits, entanglement, offset
        )
    except Exception as exc:
        # need this as Rust is now raising a QiskitError, where this function was raising ValueError
        raise ValueError(
            "Something went wrong in Rust space, here's the error:"
        ) from exc
...
def _stdlib_gate_from_simple_block(block: QuantumCircuit) -> _StdlibGateResult | None:
    ...
    if (
        ...
        or (
            getattr(
                _STANDARD_GATE_MAPPING.get(instruction.operation.name),
                "base_class",
                None,
            )
    ...
    return _StdlibGateResult(
        instruction.operation.base_class, len(instruction.operation.params)
    )
...
def _normalize_entanglement(
    ...
    if callable(entanglement):
        return lambda offset: _normalize_entanglement(
            entanglement(offset), num_entanglement_blocks
        )
...
def _normalize_blocks(
    ...
    for block in blocks:
        ...
        if isinstance(block, str):
            ...
            isinstance(block, Gate)
            and getattr(block, "_standard_gate", None) is not None
        ):
