    ...
    def __init__(
        ...
        *,
        ...
        if seed_name is None:
            ...
            seed_name = getattr(
                getattr(rng.bit_generator, "seed_seq", None), "entropy", None
            )

        super().__init__(
            num_qubits,
            name="quantum_volume_"
            + str([num_qubits, depth, seed_name]).replace(" ", ""),
        )
        if classical_permutation:
            ...
            if flatten:
                ...
                self._append(
                    CircuitInstruction(qv_circ.to_instruction(), tuple(self.qubits))
                )
        else:
            if seed is None:
                seed = seed_name
            ...
            if not flatten:
                self._append(
                    CircuitInstruction(base.to_instruction(), tuple(self.qubits))
                )
