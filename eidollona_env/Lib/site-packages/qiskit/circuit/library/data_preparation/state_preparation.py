    ...
    **References:**
    ...
    def __init__(
        ...
        if num_superpos_states <= 1:
            raise ValueError(
                "num_superpos_states must be a positive integer greater than 1."
            )
        if num_qubits is None:
            ...
            if not (
                ...
                and (num_qubits >= math.log2(num_superpos_states))
            ):
                raise ValueError(
    ...
    def _define(self):
        ...
        k = len(n_value)
        l_value = [
            index for (index, item) in enumerate(n_value) if item == 1
        ]  # Locations of '1's
