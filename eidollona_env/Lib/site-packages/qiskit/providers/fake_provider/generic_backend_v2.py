...
class GenericBackendV2(BackendV2):
        ...
        for name in self._basis_gates:
            ...
            if self._noise_info:
                ...
                qarg_set = (
                    ...
                    else range(self.num_qubits)
                )
                props = {
                    (qarg,) if isinstance(qarg, int) else qarg: None
    ...
    def _add_noisy_instruction_to_target(
        ...
        """
        qarg_set = (
            self._coupling_map if instruction.num_qubits > 1 else range(self.num_qubits)
        )
        props = {}
        for qarg in qarg_set:
            try:
    ...
    def run(self, run_input, **options):
        ...
        if not _optionals.HAS_AER:
            warnings.warn(
                "Aer not found using BasicSimulator and no noise", RuntimeWarning
            )
