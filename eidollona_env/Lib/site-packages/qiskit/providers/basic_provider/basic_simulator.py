...
class BasicSimulator(BackendV2):
    ...
    def _set_run_options(self, run_options: dict | None = None) -> None:
        ...
        if run_options.get("initial_statevector", None) is not None:
            self._initial_statevector = np.array(
                run_options["initial_statevector"], dtype=complex
            )
    ...
    def run(
        ...
        for key, value in run_options.items():
            if not hasattr(self.options, key):
                warnings.warn(
                    f"Option {key} is not used by this backend",
                    UserWarning,
                    stacklevel=2,
                )
            else:
    ...
    def _run_circuit(self, circuit) -> dict:
        ...
        for _ in range(shots):
            ...
            for operation in circuit.data:
                ...
                # Check if single qubit gate
                    ...
                    qubit0 = qubits[0]
                    qubit1 = qubits[1]
                    gate = TWO_QUBIT_GATES_WITH_PARAMETERS[operation.name](
                        *params
                    ).to_matrix()
                ...
                # Check if measure
                    ...
                    qubit = [circuit.find_bit(bit).index for bit in operation.qubits][0]
                    cmembit = [circuit.find_bit(bit).index for bit in operation.clbits][
                        0
                    ]
                    if self._sample_measure:
        ...
        header = {
            ...
            "qubit_labels": [
                [qreg.name, j] for qreg in circuit.qregs for j in range(qreg.size)
            ],
            "clbit_labels": [
                [creg.name, j] for creg in circuit.cregs for j in range(creg.size)
            ],
