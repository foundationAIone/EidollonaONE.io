...
class Exporter:
    ...
    def __init__(
        ...
        self.annotation_handlers = (
            {} if annotation_handlers is None else annotation_handlers
        )
...
class QASM3Builder:
        ...
        for stretch in self.scope.circuit.iter_declared_stretches():
            statements.append(
                ast.StretchDeclaration(
                    self.symbols.register_variable(
                        stretch.name, stretch, allow_rename=True
                    ),
                )
            )
    ...
    def build_box(self, instruction: CircuitInstruction) -> ast.BoxStatement:
        ...
        duration = self.build_duration(
            instruction.operation.duration, instruction.operation.unit
        )
    ...
    def build_annotation(self, annotation: Annotation) -> ast.Annotation:
        ...
        for namespace in iter_namespaces(annotation.namespace):
            if (
                serializer := self.annotation_handlers.get(namespace, None)
            ) is not None and (
                payload := serializer.dump(annotation)
            ) is not NotImplemented:
                return ast.Annotation(annotation.namespace, payload)
        raise QASM3ExporterError(
            f"No configured annotation serializer could handle {annotation}"
        )
        ...
        if node.type.kind is types.Duration:
            ...
            if unit == "ns":
                return ast.DurationLiteral(
                    node.value.value(), ast.DurationUnit.NANOSECOND
                )
            if unit == "us":
                return ast.DurationLiteral(
                    node.value.value(), ast.DurationUnit.MICROSECOND
                )
            if unit == "ms":
                return ast.DurationLiteral(
                    node.value.value(), ast.DurationUnit.MILLISECOND
                )
            if unit == "s":
