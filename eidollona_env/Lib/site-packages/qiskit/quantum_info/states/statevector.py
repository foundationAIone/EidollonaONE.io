...
class Statevector(QuantumState, TolerancesMixin):
    ...
    def __init__(
        ...
        super().__init__(
            op_shape=OpShape.auto(shape=shape, dims_l=dims, num_qubits_r=0)
        )
    ...
    def __getitem__(self, key: int | str) -> np.complex128:
        ...
        if isinstance(key, str):
            try:
                ...
                raise QiskitError(
                    f"Key '{key}' is not a valid binary string."
                ) from None
        if isinstance(key, int):
            if key >= self.dim:
                raise QiskitError(
                    f"Key {key} is greater than Statevector dimension {self.dim}."
                )
            if key < 0:
    ...
    def evolve(
        ...
        if isinstance(other, Instruction):
            if self.num_qubits is None:
                raise QiskitError(
                    "Cannot apply QuantumCircuit to non-qubit Statevector."
                )
    ...
    def reset(self, qargs: list[int] | None = None) -> Statevector:
        ...
        return self.evolve(
            Operator(reset, input_dims=dims, output_dims=dims), qargs=qargs
        )
    ...
    def _evolve_instruction(statevec, obj, qargs=None):
        ...
        if isinstance(obj, Initialize):
            ...
            # was cast to a complex and we cannot do an int typecheck anyways
                ...
                initialization = Statevector.from_int(
                    state, (2,) * obj.num_qubits
                )._data
            ...
            if qargs is None:
                ...
                statevec = Statevector._evolve_operator(
                    statevec, Operator(mat), qargs=qargs
                )
        ...
        for instruction in obj.definition:
            ...
            Statevector._evolve_instruction(
                statevec, instruction.operation, qargs=new_qargs
            )
        return statevec
