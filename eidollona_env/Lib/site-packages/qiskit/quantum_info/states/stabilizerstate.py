...
class StabilizerState(QuantumState):
    ...
    def __init__(
        ...
        # Initialize
        super().__init__(
            op_shape=OpShape.auto(num_qubits_r=self._data.num_qubits, num_qubits_l=0)
        )
    ...
    def expectation_value(
        self, oper: Pauli | SparsePauliOp, qargs: None | list = None
    ) -> complex:
    ...
    def _expectation_value_pauli(
        self, oper: Pauli, qargs: None | list = None
    ) -> complex:
    ...
    def probabilities_dict(
        ...
        """
        return self._get_probabilities_dict(
            outcome_bitstring=None, qargs=qargs, decimals=decimals
        )
        ...
        if z_anticommuting == 0:
            ...
            for i in range(num_qubits):
                if clifford.x[i][qubit]:
                    aux_pauli = self._rowsum_deterministic(
                        clifford, aux_pauli, i + num_qubits
                    )
            ...
            for i in range(2 * num_qubits):
                ...
                if (
                    (clifford.x[i][qubit])
                    and (i != p_qubit)
                    and (i != (p_qubit - num_qubits))
                ):
                    self._rowsum_nondeterministic(clifford, i, p_qubit)

            clifford.destab[p_qubit - num_qubits] = clifford.stab[
                p_qubit - num_qubits
            ].copy()
    ...
    def _get_probabilities_dict(
        ...
        probs: dict[str, float] = (
            {}
        )  # Probabilities dict to return with the measured values
