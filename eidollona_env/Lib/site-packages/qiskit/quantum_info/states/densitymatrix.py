...
class DensityMatrix(QuantumState, TolerancesMixin):
    ...
    def __init__(
        ...
        super().__init__(
            op_shape=OpShape.auto(shape=self._data.shape, dims_l=dims, dims_r=dims)
        )
    ...
    def evolve(
        self,
        other: (
            Operator | QuantumChannel | circuit.instruction.Instruction | QuantumCircuit
        ),
    ...
    def _expectation_value_pauli(self, pauli, qargs=None):
        ...
        if x_mask == 0:
            return pauli_phase * density_expval_pauli_no_x(
                data, self.num_qubits, z_mask
            )
    ...
    def expectation_value(
        self, oper: Operator, qargs: None | list[int] = None
    ) -> complex:
