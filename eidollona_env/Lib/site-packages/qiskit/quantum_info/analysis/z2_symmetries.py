...
class Z2Symmetries:
    ...
    def is_empty(self) -> bool:
        ...
        """
        return (
            len(self._symmetries) == 0
            or len(self._sq_paulis) == 0
            or len(self._sq_list) == 0
        )
    ...
    def find_z2_symmetries(cls, operator: SparsePauliOp) -> Z2Symmetries:
        ...
        for pauli in iter(operator):
            stacked_paulis.append(
                np.concatenate((pauli.paulis.x[0], pauli.paulis.z[0]), axis=0).astype(
                    int
                )
            )
        ...
        def _test_symmetry_row_col(
            row: int, col: int, idx_test: list, row_test: list
        ) -> bool:
            """
            ...
            for col in range(half_symm_shape):
                for key in ("Z_or_I", "X_or_I", "Y_or_I"):
                    ...
                    if current_test_result:
                        sq_paulis.append(
                            Pauli(
                                (np.zeros(half_symm_shape), np.zeros(half_symm_shape))
                            )
                        )
    ...
    def taper_clifford(
        self, operator: SparsePauliOp
    ) -> Union[SparsePauliOp, list[SparsePauliOp]]:
    ...
    def _taper(
        self, op: SparsePauliOp, curr_tapering_values: list[int]
    ) -> SparsePauliOp:
        pauli_list = []
