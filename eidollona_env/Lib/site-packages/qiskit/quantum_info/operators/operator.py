...
class Operator(LinearOp):
    ...
    def apply_permutation(self, perm: list, front: bool = False) -> Operator:
        ...
        if front:
            ...
            axes_r = tuple(
                self._op_shape._num_qargs_l + x for x in (np.argsort(perm[::-1]))[::-1]
            )

            # updated shape: original on left, permuted on right
            new_shape_l = self._op_shape.dims_l()
            new_shape_r = tuple(
                raw_shape_r[n_dims_r - n - 1] for n in reversed(inv_perm)
            )
            ...
            axes_r = tuple(
                self._op_shape._num_qargs_l + x
                for x in range(self._op_shape._num_qargs_r)
            )
        ...
        new_mat = (
            self._data.reshape(split_shape)
            .transpose(axes_order)
            .reshape(self._op_shape.shape)
        )
    ...
    def compose(
        self, other: Operator, qargs: list | None = None, front: bool = False
    ) -> Operator:
        if qargs is None:
    ...
    def power(
        ...
        if isinstance(n, int):
            ...
            if assume_unitary:
                ...
                decomposition_power = [
                    pow(element, n) for element in decomposition_diagonal
                ]
                unitary_power = (
                    unitary @ np.diag(decomposition_power) @ unitary.conj().T
                )
    ...
    def equiv(
        self, other: Operator, rtol: float | None = None, atol: float | None = None
    ) -> bool:
    ...
    def _instruction_to_matrix(cls, obj):
        ...
        if not isinstance(obj, (Instruction, Clifford, AnnotatedOperation)):
            raise QiskitError(
                "Input is neither Instruction, Clifford or AnnotatedOperation."
            )
        mat = None
    ...
    def _append_instruction(self, obj, qargs=None):
        ...
        if mat is not None:
            ...
            if obj.definition.global_phase:
                ...
                op = self.compose(
                    ScalarOp(
                        dimension, np.exp(1j * float(obj.definition.global_phase))
                    ),
                    qargs=qargs,
                )
                self._data = op.data
