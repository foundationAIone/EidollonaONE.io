...
class Kraus(QuantumChannel):
    ...
    def __init__(
        self,
        data: (
            QuantumCircuit | circuit.instruction.Instruction | BaseOperator | np.ndarray
        ),
        ...
        if isinstance(data, (list, tuple, np.ndarray)):
            ...
            op_shape = OpShape.auto(
                dims_l=output_dims, dims_r=input_dims, shape=kraus[0][0].shape
            )
        else:
    ...
    def compose(
        self, other: Kraus, qargs: list | None = None, front: bool = False
    ) -> Kraus:
        if qargs is None:
