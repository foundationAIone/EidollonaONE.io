...
_PARITY = np.array(
    [-1 if bin(i).count("1") % 2 else 1 for i in range(256)], dtype=complex
)
...
class BasePauli(BaseOperator, AdjointMixin, MultiplyMixin):
    ...
    def compose(
        self, other, qargs: list | None = None, front: bool = False, inplace=False
    ):
    ...
    def _stack(array, size, vertical=True):
        ...
        if size == 1:
            return array
        if vertical:
            return np.vstack(size * [array]).reshape(
                (size * len(array),) + array.shape[1:]
            )
    ...
    def _to_label(z, x, phase, group_phase=False, full_group=True, return_phase=False):
        ...
        phase = (
            int(phase) if group_phase else int(phase) - ascii_label.count(b"Y")
        ) % 4
...
def _evolve_cy(base_pauli, qctrl, qtrgt):
    ...
    base_pauli._phase += x1 + 2 * np.logical_and(x1, x2).T.astype(
