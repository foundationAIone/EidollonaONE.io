...
class Clifford(BaseOperator, AdjointMixin, Operation):
    ...
    **Circuit Conversion**
    ...
    def _compose_general(cls, first, second):
        ...
        phase = (
            (
                np.matmul(second.symplectic_matrix, first.phase, dtype=int)
                + second.phase
                + p
            )
            % 2
        ...
        data = cls._stack_table_phase(
            (
                np.matmul(second.symplectic_matrix, first.symplectic_matrix, dtype=int)
                % 2
            ).astype(bool),
            phase,
        )
    ...
    def _pad_with_identity(self, clifford, qargs):
        ...
        padded = Clifford(
            np.eye(2 * self.num_qubits, dtype=bool), validate=False, copy=False
        )
    ...
    def _pauli_matrix_to_row(mat, num_qubits):
        ...
        def bitvector(n, num_bits):
            return np.array(
                [int(digit) for digit in format(n, f"0{num_bits}b")], dtype=bool
            )[::-1]
        ...
        expected = coef * functools.reduce(
            np.kron, [zvec if z else ivec for z in zbits[::-1]]
        )
