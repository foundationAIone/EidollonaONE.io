...
class Pauli(BasePauli):
    ...
    **Data Access**
    ...
    def __init__(
        self, data: str | tuple | Pauli | ScalarOp | QuantumCircuit | None = None
    ):
    ...
    def insert(self, qubits: int | list, value: Pauli) -> Pauli:
        ...
        ret = Pauli(
            (np.zeros(ret_qubits, dtype=bool), np.zeros(ret_qubits, dtype=bool))
        )
        if isinstance(qubits, (int, np.integer)):
    ...
    def dot(
        self, other: Pauli, qargs: list | None = None, inplace: bool = False
    ) -> Pauli:
    ...
    def _from_label(label):
        ...
        pauli_bytes = np.frombuffer(match_["pauli"].encode("ascii"), dtype=np.uint8)[
            ::-1
        ]
