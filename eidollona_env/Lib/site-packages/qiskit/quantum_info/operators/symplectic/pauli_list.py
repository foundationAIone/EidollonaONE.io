...
class PauliList(BasePauli, LinearMixin, GroupMixin):
    ...
    **Iteration**
    ...
    def __setitem__(self, index, value):
        ...
        phase = (
            value._phase.item() if isinstance(row, (int, np.integer)) else value._phase
        )
    ...
    def unique(
        self, return_index: bool = False, return_counts: bool = False
    ) -> PauliList:
        ...
        **Example**
        ...
        if np.any(self._phase != self._phase[0]):
            ...
            array = np.hstack(
                [self._z, self._x, self.phase.reshape((self.phase.shape[0], 1))]
            )
        else:
        ...
        if return_counts:
            _, index, counts = np.unique(
                array, return_index=True, return_counts=True, axis=0
            )
        else:
    ...
    def compose(
        ...
        return PauliList(
            super().compose(other, qargs=qargs, front=front, inplace=inplace)
        )

    def dot(
        self, other: PauliList, qargs: None | list = None, inplace: bool = False
    ) -> PauliList:
    ...
    def label_iter(self):
        ...
        class LabelIterator(CustomIterator):
            ...
            def __getitem__(self, key):
                return self.obj._to_label(
                    self.obj._z[key], self.obj._x[key], self.obj._phase[key]
                )
    ...
    def from_symplectic(
        ...
        if isinstance(phase, np.ndarray) and np.ndim(phase) > 1:
            raise ValueError(
                f"phase should be at most 1D but has {np.ndim(phase)} dimensions."
            )
