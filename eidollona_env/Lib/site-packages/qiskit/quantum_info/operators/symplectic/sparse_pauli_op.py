...
class SparsePauliOp(LinearOp):
    ...
    def __init__(
        ...
        *,
        ...
        if ignore_pauli_phase and not isinstance(data, PauliList):
            raise QiskitError(
                "ignore_pauli_phase=True is only valid with PauliList data"
            )
    ...
    def _multiply(self, other):
        if not isinstance(other, (Number, ParameterExpression)):
            raise QiskitError(
                "other is neither a Number nor a Parameter/ParameterExpression"
            )
        if other == 0:
    ...
    def simplify(
        ...
        if self.coeffs.dtype == object:
            ...
            non_zero = np.logical_not(
                np.isclose(
                    [to_complex(x) for x in self.coeffs], 0, atol=atol, rtol=rtol
                )
            )
        else:
        ...
        if self.coeffs.dtype == object:
            is_zero = np.array(
                [
                    np.isclose(to_complex(coeff), 0, atol=atol, rtol=rtol)
                    for coeff in coeffs
                ]
            )
        else:
    ...
    def from_operator(
        ...
        return SparsePauliOp(
            pauli_list, zx_paulis.coeffs, ignore_pauli_phase=True, copy=False
        )
    ...
    def to_list(self, array: bool = False):
        ...
        labels = np.zeros(
            self.size,
            dtype=[("labels", pauli_labels.dtype), ("coeffs", self.coeffs.dtype)],
        )
