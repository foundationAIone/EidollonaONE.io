...
def synth_mcx_1_clean_b95(num_ctrl_qubits: int) -> QuantumCircuit:
    ...
    qubits1 = [
        *controls1,
        q_ancilla,
        *q[middle : middle + mcx1.num_qubits - len(controls1) - 1],
    ]
...
def _linear_depth_ladder_ops(
    num_ladder_qubits: int,
...
def synth_mcx_1_kg24(num_ctrl_qubits: int, clean: bool = True) -> QuantumCircuit:
    ...
    if num_ctrl_qubits <= 2:
        raise QiskitError(
            "kg24 synthesis requires at least 3 control qubits. Use CCX directly."
        )
    ...
    qc.rccx(
        q_controls[0], q_controls[1], q_ancilla[0]
    ...
    while len(ctrls) > 1:
        ...
        while len(nxt_batch) > 1:
            ...
            if ccx_t != [ancilla_idx]:
                qc.compose(
                    _n_parallel_ccx_x(ccx_n), ccx_x + ccx_y + ccx_t, inplace=True
                )
            else:
            ...
            new_anc += nxt_batch[
                st:
            ]  #                     # newly created cond. clean ancilla
...
def synth_mcx_2_kg24(num_ctrl_qubits: int, clean: bool = True) -> QuantumCircuit:
    ...
    if not clean:
        ...
        qc.compose(
            ladder_ops_new.inverse(), q_control[:] + [q_ancilla[0]], inplace=True
        )
