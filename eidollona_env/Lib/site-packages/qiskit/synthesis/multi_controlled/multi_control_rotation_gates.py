...
def _mcsu2_real_diagonal(
    ...
    if isinstance(gate, RYGate):
        ...
        if unitary.shape != (2, 2):
            raise QiskitError(
                f"The unitary must be a 2x2 matrix, but has shape {unitary.shape}."
            )

        if not is_unitary_matrix(unitary):
            raise QiskitError(
                f"The unitary in must be an unitary matrix, but is {unitary}."
            )
        ...
        if np.isclose(z, -1):
            ...
            alpha_i = z.imag / (
                2.0
                * math.sqrt((z.real + 1.0) * (math.sqrt((z.real + 1.0) / 2.0) + 1.0))
            )
    ...
    if use_basis_gates:
        circuit = transpile(
            circuit, basis_gates=["p", "u", "cx"], qubits_initially_zero=False
        )
