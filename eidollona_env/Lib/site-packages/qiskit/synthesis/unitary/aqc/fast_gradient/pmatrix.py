...
class PMatrix:
    ...
    def mul_right_q1(self, layer: Layer1Q, temp_mat: np.ndarray, dagger: bool):
        ...
        np.take(
            mat,
            np.take(self._right_perm, perm, out=self._temp_perm),
            axis=1,
            out=temp_mat,
        )
    ...
    def mul_left_q1(self, layer: Layer1Q, temp_mat: np.ndarray):
        ...
        np.take(
            mat,
            np.take(self._left_perm, perm, out=self._temp_perm),
            axis=0,
            out=temp_mat,
        )
        ...
        if dim > 512:
            ...
            half = dim // 2
            np.copyto(
                mat.reshape((2, half, dim)),
                np.swapaxes(temp_mat.reshape((half, 2, dim)), 0, 1),
            )
            np.dot(gmat, mat.reshape(2, -1), out=temp_mat.reshape(2, -1))
            np.copyto(
                mat.reshape((half, 2, dim)),
                np.swapaxes(temp_mat.reshape((2, half, dim)), 0, 1),
            )
    ...
    def mul_left_q2(self, layer: Layer2Q, temp_mat: np.ndarray):
        ...
        if dim > 512:
            ...
            half = dim // 4
            np.copyto(
                mat.reshape((4, half, dim)),
                np.swapaxes(temp_mat.reshape((half, 4, dim)), 0, 1),
            )
            np.dot(gmat, mat.reshape(4, -1), out=temp_mat.reshape(4, -1))
            np.copyto(
                mat.reshape((half, 4, dim)),
                np.swapaxes(temp_mat.reshape((4, half, dim)), 0, 1),
            )
    ...
    def product_q1(
        self, layer: Layer1Q, tmp1: np.ndarray, tmp2: np.ndarray
    ) -> np.complex128:
        """
    ...
    def product_q2(
        self, layer: Layer2Q, tmp1: np.ndarray, tmp2: np.ndarray
    ) -> np.complex128:
        """
        Computes and returns: ``Trace(mat @ C) = Trace(mat @ P^T @ gmat @ P) =
        Trace((P @ mat @ P^T) @ gmat) = Trace(C @ (P @ mat @ P^T)) =
