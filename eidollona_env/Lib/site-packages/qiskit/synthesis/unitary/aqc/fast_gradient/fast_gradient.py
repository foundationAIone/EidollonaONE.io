...
class FastCNOTUnitObjective(CNOTUnitObjective):
    ...
    def __init__(self, num_qubits: int, cnots: np.ndarray):
        ...
        # 4x4 C-gate matrices:
        self._c_gates = np.full(
            (self.num_cnots, 4, 4), fill_value=0, dtype=np.complex128
        )
        # derivatives of 4x4 C-gate matrices:
        self._c_dervs = np.full(
            (self.num_cnots, 4, 4, 4), fill_value=0, dtype=np.complex128
        )
        # 4x4 F-gate matrices:
        self._f_gates = np.full((num_qubits, 2, 2), fill_value=0, dtype=np.complex128)
        # derivatives of 4x4 F-gate matrices:
        self._f_dervs = np.full(
            (num_qubits, 3, 2, 2), fill_value=0, dtype=np.complex128
        )
