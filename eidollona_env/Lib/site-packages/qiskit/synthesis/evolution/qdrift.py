...
class QDrift(ProductFormula):
    ...
    def __init__(
        ...
        atomic_evolution: (
            Callable[
                [QuantumCircuit, qiskit.quantum_info.Pauli | SparsePauliOp, float], None
            ]
            | None
        ) = None,
        seed: int | None = None,
    ...
    def expand(
        ...
        if isinstance(operators, list):
            paulis = list(
                chain.from_iterable([op.to_sparse_list() for op in operators])
            )
        else:
        ...
        sampled_paulis = [
            (pauli[0], pauli[1], np.real(np.sign(pauli[2])) * rescaled_time)
            for pauli in sampled
        ]
