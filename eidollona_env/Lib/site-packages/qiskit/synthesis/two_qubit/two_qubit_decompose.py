...
def decompose_two_qubit_product_gate(special_unitary_matrix: np.ndarray):
    ...
    (L, R, phase) = two_qubit_decompose.decompose_two_qubit_product_gate(
...
class TwoQubitWeylDecomposition:
    ...
    def __repr__(self):
        ...
        indent = "\n" + " " * 4
        specialization_variant = str(self._inner_decomposition.specialization).split(
            "."
        )[1]
        specialization_repr = (
            f"{type(self).__qualname__}._specializations.{specialization_variant}"
        )
        lines = (
            [prefix]
            + pretty
            ...
            for gate, params, qubits in sequence:
                if gate is None:
                    dag.apply_operation_back(
                        self.gate, tuple(q[x] for x in qubits), check=False
                    )
                else:
