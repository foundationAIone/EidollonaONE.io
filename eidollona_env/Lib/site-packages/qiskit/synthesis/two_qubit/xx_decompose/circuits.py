...
def decompose_xxyy_into_xxyy_xx(a_target, b_target, a_source, b_source, interaction):
    ...
    if (
        abs(cmath.phase(generated_matrix[3, 0]) - np.pi / 2) < 0.01
    ...
    for source_reflection_name in reflection_options:
        ...
        for source_shift_name in shift_options:
            ...
            for i, j in [
                ...
                if (
                    abs(np.mod(abs(shifted_source_coord[i] - target[j]), np.pi))
                    < EPSILON
                    or abs(
                        np.mod(abs(shifted_source_coord[i] - target[j]), np.pi) - np.pi
                    )
                    < EPSILON
                ):
