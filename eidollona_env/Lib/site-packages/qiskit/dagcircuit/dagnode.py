...
def _legacy_condition_eq(cond1, cond2, bit_indices1, bit_indices2) -> bool:
    ...
    if isinstance(target1, ClassicalRegister) and isinstance(
        target2, ClassicalRegister
    ):
...
def _circuit_to_dag(
    circuit: QuantumCircuit, node_qargs, node_cargs, bit_indices
) -> DAGCircuit:
    ...
    return circuit_to_dag(
        ...
        qubit_order=[
            inner
            for _outer, inner in sorted(zip(node_qargs, circuit.qubits), key=sort_key)
        ],
        clbit_order=[
            inner
            for _outer, inner in sorted(zip(node_cargs, circuit.clbits), key=sort_key)
        ],
    )
...
def _switch_case_eq(node1, node2, bit_indices1, bit_indices2):
    ...
    if isinstance(target1, expr.Expr) and isinstance(target2, expr.Expr):
        ...
        if bit_indices1[target1] != bit_indices2[target2]:
            return False
    elif isinstance(target1, ClassicalRegister) and isinstance(
        target2, ClassicalRegister
    ):
    ...
    return (
        ...
        and all(
            set(labels1) == set(labels2) for labels1, labels2 in zip(cases1, cases2)
        )
...
def _for_loop_eq(node1, node2, bit_indices1, bit_indices2):
    ...
    if indexset1 != indexset2:
        return False
    if (param1 is None and param2 is not None) or (
        param1 is not None and param2 is None
    ):
        return False
    ...
    return _circuit_to_dag(
        body1, node1.qargs, node1.cargs, bit_indices1
    ) == _circuit_to_dag(body2, node2.qargs, node2.cargs, bit_indices2)
...
def _box_eq(node1, node2, bit_indices1, bit_indices2):
    return (
        ...
        and (
            ...
            == _circuit_to_dag(
                node2.op.blocks[0], node2.qargs, node2.cargs, bit_indices2
            )
        )
