...
class _DAGDependencyV2:
    ...
    **Reference:**
        ...
        try:
            if isinstance(bit, Qubit):
                ...
                raise DAGDependencyError(
                    f"Could not locate bit of unknown type: {type(bit)}"
                )
            ...
            if reachable[prev_node_id]:
                ...
                if not self.comm_checker.commute(
                    ...
                    self._multi_graph.add_edge(
                        prev_node_id, max_node_id, {"commute": False}
                    )

                    predecessor_ids = sorted(
                        [
                            node._node_id
                            for node in self.predecessors(self._get_node(prev_node_id))
                        ]
                    )
    ...
    def topological_nodes(self, key=None) -> Generator[DAGOpNode, Any, Any]:
        ...
        def _key(x):
            return ",".join(
                f"{self.find_bit(q).index:04d}"
                for q in itertools.chain(x.qargs, x.cargs)
            )
    ...
    def ancestors(self, node):
        ...
        return {
            self._multi_graph[x] for x in rx.ancestors(self._multi_graph, node._node_id)
        }
