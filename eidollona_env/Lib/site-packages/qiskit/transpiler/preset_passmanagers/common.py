...
_ControlFlowState = collections.namedtuple(
    "_ControlFlowState", ("working", "not_working")
)
...
_CONTROL_FLOW_STATES = {
    ...
    "routing_method": _ControlFlowState(
        working={"default", "none", "stochastic", "sabre"},
        not_working={"lookahead", "basic"},
    ),
...
class _InvalidControlFlowForBackend:
    ...
    def __init__(self, basis_gates=(), target=None):
        if target is not None and len(target.operation_names) > 0:
            ...
            self.unsupported = [
                op for op in CONTROL_FLOW_OP_NAMES if op not in basis_gates
            ]
        else:
...
def generate_control_flow_options_check(
    ...
    out.append(
        ConditionalController(
            Error(backend_control.message), condition=backend_control.condition
        )
    )
    return out
...
def _layout_not_perfect(property_set):
    ...
    return (
        property_set["is_swap_mapped"] is not None
        and not property_set["is_swap_mapped"]
    )
...
def generate_routing_passmanager(
    ...
    if target is not None and is_vf2_fully_bounded:
        ...
        routing.append(
            ConditionalController(ApplyLayout(), condition=_apply_post_layout_condition)
        )
...
def generate_scheduling(
    ...
    if scheduling_method:
        ...
        try:
            scheduling.append(
                scheduler[scheduling_method](instruction_durations, target=target)
            )
        except KeyError as ex:
            raise TranspilerError(
                f"Invalid scheduling method {scheduling_method}."
            ) from ex
...
def is_clifford_t_basis(basis_gates=None, target=None) -> bool:
    ...
    if (basis_gates is None) or (
        ("t" not in basis_gates) and ("tdg" not in basis_gates)
    ):
        return False
