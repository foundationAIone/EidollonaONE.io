...
class PassManager(BasePassManager):
    ...
    def _finalize_layouts(self, dag):
        if (
            virtual_permutation_layout := self.property_set[
...
class StagedPassManager(PassManager):
    ...
    def _validate_stages(self, stages: Iterable[str]) -> None:
        invalid_stages = [
            stage
            for stage in stages
            if self.invalid_stage_regex.search(stage) is not None
        ]
        if invalid_stages:
            with io.StringIO() as msg:
                msg.write(
                    f"The following stage names are not valid: {invalid_stages[0]}"
                )
    ...
    def __setattr__(self, attr, value):
        if value == self and attr in self.expanded_stages:
            raise TranspilerError(
                "Recursive definition of StagedPassManager disallowed."
            )
