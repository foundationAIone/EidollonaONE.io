...
class SwapStrategy:
    ...
    def __init__(
        ...
        for i, layer in enumerate(self._swap_layers):
            ...
            if len(layer_qubits) != len(set(layer_qubits)):
                raise QiskitError(
                    f"The {i}th swap layer contains a qubit with multiple swaps."
                )

    @classmethod
    def from_line(
        cls, line: list[int], num_swap_layers: int | None = None
    ) -> "SwapStrategy":
        ...
        if len(line) < 2:
            raise ValueError(
                f"The line cannot have less than two elements, but is {line}"
            )
    ...
    def distance_matrix(self) -> np.ndarray:
        ...
        if self._distance_matrix is None:
            self._distance_matrix = np.full(
                (self._num_vertices, self._num_vertices), -1, dtype=int
            )
    ...
    def swapped_coupling_map(self, idx: int) -> CouplingMap:
        ...
        edges = [
            [permutation[i], permutation[j]] for i, j in self._coupling_map.get_edges()
        ]
