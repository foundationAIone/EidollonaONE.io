...
from qiskit.transpiler.passes.routing.commuting_2q_gate_routing.swap_strategy import (
    SwapStrategy,
)
    ...
    def run(self, dag: DAGCircuit) -> DAGCircuit:
        ...
        for node in dag.topological_op_nodes():
            if isinstance(node.op, Commuting2qBlock):
                ...
                new_dag.compose(
                    self.swap_decompose(dag, node, current_layout, swap_strategy)
                )
            else:
    ...
    def swap_decompose(
        ...
        for i in range(max_distance + 1):
            ...
            for (j, k), local_gate in gate_layers.get(i, {}).items():
                current_layer[self._position_in_cmap(dag, j, k, current_layout)] = (
                    local_gate
                )
