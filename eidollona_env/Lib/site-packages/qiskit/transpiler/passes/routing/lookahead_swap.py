...
_Step = collections.namedtuple(
    "_Step", ("state", "swaps_added", "gates_mapped", "gates_remaining")
)
...
class LookaheadSwap(TransformationPass):
    ...
    def run(self, dag):
        ...
        while gates_remaining:
            logger.debug(
                "Top-level routing step: %d gates remaining.", len(gates_remaining)
            )
...
def _search_forward_n_swaps(state, gates, depth, width):
    ...
    if state.swaps is None:
        ...
        state = state._replace(
            swaps={
                ((a, b) if a < b else (b, a)) for a, b in state.coupling_map.get_edges()
            }
        )
    ...
    for rank, (_, swap, new_state) in enumerate(ranked_swaps):
        next_step = _search_forward_n_swaps(
            new_state, gates_remaining, depth - 1, width
        )
...
def _map_free_gates(state, gates):
    ...
    for gate in gates:
        ...
        if blocked_qubits.intersection(qubits):
            ...
            state.coupling_map.distance(layout_map[qubits[0]], layout_map[qubits[1]])
            == 1
        ):
            mapped_gate = _transform_gate_for_system(gate, state)
            mapped_gates.append(mapped_gate)
...
def _calc_layout_distance(gates, state, max_gates=None):
    ...
    for gate in gates[:max_gates]:
        ...
        qubits = gate["partition"][0]
        if len(qubits) == 2:
            out += state.coupling_map.distance(
...
def _transform_gate_for_system(gate, state):
    ...
    mapped_op_node.qargs = tuple(
        device_qreg[layout_map[a]] for a in mapped_op_node.qargs
    )
