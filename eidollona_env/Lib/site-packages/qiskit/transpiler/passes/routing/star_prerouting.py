...
class StarPreRouting(TransformationPass):
    ...
    def _direct_preds(self, dag, node):
        ...
        if not isinstance(dag, DAGDependency):
            return [
                pred for pred in dag.predecessors(node) if isinstance(pred, DAGOpNode)
            ]
        else:
            return [
                dag.get_node(pred_id)
                for pred_id in dag.direct_predecessors(node.node_id)
            ]
    ...
    def _direct_succs(self, dag, node):
        ...
        if not isinstance(dag, DAGDependency):
            ...
            return [
                dag.get_node(succ_id) for succ_id in dag.direct_successors(node.node_id)
            ]
    ...
    def run(self, dag):
        ...
        star_blocks, processing_order = self.determine_star_blocks_processing(
            dag, min_block_size=2
        )
    ...
    def star_preroute(self, dag, blocks, processing_order):
        ...
        def _apply_mapping(qargs, qubit_mapping, qubits):
            return tuple(
                qubits[qubit_mapping[dag.find_bit(qubit).index]] for qubit in qargs
            )
        ...
        def tie_breaker_key(node):
            ...
            return ",".join(
                f"{dag.find_bit(q).index:04d}"
                for q in itertools.chain(node.qargs, node.cargs)
            )
        ...
        for node in dag.topological_op_nodes(key=tie_breaker_key):
            ...
            if block_id is not None:
                ...
                for inner_node in sequence:
                    ...
                    if not inner_node is last_2q_gate and not isinstance(
                        inner_node.op, Barrier
                    ):
                        new_dag.apply_operation_back(
                            SwapGate(),
                            _apply_mapping(inner_node.qargs, qubit_mapping, dag.qubits),
