...
class TranslateParameterizedGates(TransformationPass):
    ...
    def __init__(
        ...
        if target is not None:
            ...
            raise ValueError(
                "One of ``supported_gates`` or ``target`` must be specified."
            )

        self._supported_gates = supported_gates
        self._target = target
        self._translator = BasisTranslator(
            equivalence_library, supported_gates, target=target
        )
...
def _is_supported(
    node: DAGOpNode, supported_gates: list[str], target: Target | None
) -> bool:
