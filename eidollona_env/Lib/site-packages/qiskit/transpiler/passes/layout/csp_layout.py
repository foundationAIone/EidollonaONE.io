...
class CSPLayout(AnalysisPass):
    ...
    def run(self, dag):
        ...
        if self.time_limit is None and self.call_limit is None:
            ...
            solver = CustomSolver(
                call_limit=self.call_limit, time_limit=self.time_limit
            )
        ...
        problem.addConstraint(
            AllDifferentConstraint()
        )  # each wire is map to a single qubit
        ...
        if solution is None:
            ...
            if isinstance(solver, CustomSolver):
                if (
                    solver.time_current is not None
                    and solver.time_current >= self.time_limit
                ):
                    stop_reason = "time limit reached"
                elif (
                    solver.call_current is not None
                    and solver.call_current >= self.call_limit
                ):
                    stop_reason = "call limit reached"
        else:
            stop_reason = "solution found"
            self.property_set["layout"] = Layout(
                {v: qubits[k] for k, v in solution.items()}
            )
