...
class Optimize1qGatesDecomposition(TransformationPass):
    ...
    def __init__(self, basis=None, target=None):
        ...
        self._target = (
            target if target is not None and len(target.operation_names) > 0 else None
        )
    ...
    def _build_error_map(self):
        ...
        if self._target is not None and self._target.num_qubits is not None:
            error_map = euler_one_qubit_decomposer.OneQubitGateErrorMap(
                self._target.num_qubits
            )
        ...
        if (
            ...
            if qubits_tuple in self._local_decomposers_cache:
                ...
                available_1q_basis = set(
                    self._target.operation_names_for_qargs(qubits_tuple)
                )
    ...
    def _substitution_checks(
        self, old_run, new_circ, basis, qubit, old_error=None, new_error=None
    ):
        """
