...
class Optimize1qGates(TransformationPass):
    ...
    def run(self, dag):
        ...
        for run in runs:
            ...
            if self.target is not None:
                if right_name == "u2" and not self.target.instruction_supported(
                    "u2", run_qubits
                ):
            ...
            if right_name == "u3":
                if "u3" in self.basis:
                    ...
                    raise TranspilerError(
                        f"It was not possible to use the basis {self.basis}"
                    )
    ...
    def compose_u3(theta1, phi1, lambda1, theta2, phi2, lambda2):
        ...
        """
        (theta, phi, lamb) = compose_u3_rust(
            theta1, phi1, lambda1, theta2, phi2, lambda2
        )
...
def _split_runs_on_parameters(runs):
    ...
    for run in runs:
        ...
        groups = groupby(
            run, lambda x: x.op.is_parameterized() and x.op.name in ("u3", "u")
        )
