...
class LightCone(TransformationPass):
    ...
    def __init__(
        self, bit_terms: str | None = None, indices: list[int] | None = None
    ) -> None:
        """
        Args:
        ...
        if bit_terms is not None:
            if not indices:
                raise ValueError(
                    "`indices` must be non-empty when providing `bit_terms`."
                )
    ...
    def _get_initial_lightcone(
        ...
        if self.bit_terms is None:
            lightcone_operations = [
                (ZGate(), [qubit_index]) for qubit_index in lightcone_qubits
            ]
        else:
