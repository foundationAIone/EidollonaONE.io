...
class OptimizeAnnotated(TransformationPass):
    ...
    * Applies conjugate reduction to annotated operations. As an example,
    ...
    def __init__(
        ...
        self._top_level_only = not recurse or (
            self._basis_gates is None and self._target is None
        )
    ...
    def _run_inner(self, dag) -> Tuple[DAGCircuit, bool]:
        ...
        if self._top_level_only:
            ...
            if "annotated" not in op_names and not CONTROL_FLOW_OP_NAMES.intersection(
                op_names
            ):
                return dag, False
    ...
    def _conjugate_decomposition(
        ...
        while len(active_qubits) > 0:
            ...
            for q in to_check:
                ...
                if (
                    front_node.qargs != back_node.qargs
                    or front_node.cargs != back_node.cargs
                ):
                    continue
    ...
    def _conjugate_reduce_op(
        ...
        base_decomposition: Tuple[DAGCircuit, DAGCircuit, DAGCircuit],
    ) -> Operation:
        """
        ...
        q_instr = Instruction(
            name="iq",
            num_qubits=op.base_op.num_qubits,
            num_clbits=op.base_op.num_clbits,
            params=[],
        )
        ...
        qubits = circ.qubits
        circ.compose(
            dag_to_circuit(p_dag),
            qubits[num_control_qubits : op.num_qubits],
            inplace=True,
        )
        circ.append(
            AnnotatedOperation(base_op=q_instr, modifiers=op.modifiers),
            range(op.num_qubits),
        )
        circ.compose(
            dag_to_circuit(r_dag),
            qubits[num_control_qubits : op.num_qubits],
            inplace=True,
        )
