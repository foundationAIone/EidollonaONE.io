...
from qiskit.transpiler.passes.optimization.template_matching.forward_match import (
    ForwardMatch,
)
...
class TemplateMatching:
    ...
    def _list_first_match_new(
        self, node_circuit, node_template, n_qubits_t, n_clbits_t
    ):
        """
        ...
        # Controlled gate
        if (
            isinstance(node_circuit.op, ControlledGate)
            and node_template.op.num_ctrl_qubits > 1
        ):
            ...
            # Symmetric base gate
            if node_template.op.base_gate.name not in [
                "rxx",
                "ryy",
                "rzz",
                "swap",
            ...
            # Not symmetric base gate
                ...
                for control_perm_q in itertools.permutations(control_qubits_circuit):
                    control_perm_q = list(control_perm_q)
                    for not_control_perm_q in itertools.permutations(
                        not_control_qubits_circuit
                    ):
                        ...
                        for q in node_template.qindices:
                            ...
                            l_q_sub[q] = node_circuit_perm[
                                node_template.qindices.index(q)
                            ]
                        l_q.append(l_q_sub)
        # Not controlled
        else:
            # Symmetric gate
            if node_template.op.name not in [
                "rxx",
                "ryy",
                "rzz",
                "swap",
    ...
    def _sublist(self, lst, exclude, length):
        ...
        for sublist in itertools.combinations(
            [e for e in lst if e not in exclude], length
        ):
            yield list(sublist)
    ...
    def _explore_circuit(self, node_id_c, node_id_t, n_qubits_t, length):
        ...
        if 2 * len(successors_template) > len(template_nodes):
            ...
            not_successors = list(
                set(circuit_nodes)
                - set(self.circuit_dag_dep.get_node(node_id_c).successors)
            )
            candidate = [
                not_successors[j]
                for j in range(
                    len(not_successors) - 1, len(not_successors) - 1 - length, -1
                )
            ]
        ...
        for template_index in range(0, self.template_dag_dep.size()):
            for circuit_index in range(0, self.circuit_dag_dep.size()):
                ...
                if self.circuit_dag_dep.get_node(circuit_index).op.soft_compare(
                    ...
                    for sub_q in self._sublist(
                        list_circuit_q, qarg_c, n_qubits_t - len(qarg_t)
                    ):
