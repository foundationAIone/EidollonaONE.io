...
class BackwardMatch:
    ...
    def _backward_heuristics(self, gate_indices, length, survivor):
        ...
        if list_counter.count(list_counter[0]) == len(list_counter) and list_counter[
            0
        ] <= len(gate_indices):
            if (list_counter[0] - 1) % length == 0:
                ...
                largest = heapq.nlargest(
                    survivor, range(len(metrics)), key=lambda x: metrics[x]
                )
    ...
    def run_backward_match(self):
        ...
        while self.matching_list.matching_scenarios_list:
            ...
            if (
                ...
                match_store_list.append(
                    Match(matches_scenario, self.qubits, self.clbits)
                )
                continue
            ...
            candidates_indices = self._find_backward_candidates(
                template_blocked, matches_scenario
            )
            ...
            for template_id in candidates_indices:
                ...
                if (
                    ...
                    for potential_block in self.template_dag_dep.successors(
                        template_id
                    ):
                        if not template_matched_match[potential_block]:
                            ...
                            for block_id in block_list:
                                for succ_id in self.template_dag_dep.successors(
                                    block_id
                                ):
                    ...
                    if (
                        [self.node_id_t, self.node_id_c] in new_matches_scenario_match
            ...
            if global_match:
                ...
                new_matches_scenario_block_s = [
                    elem
                    for elem in matches_scenario_block_s
                ...
                if (
                    [self.node_id_t, self.node_id_c] in new_matches_scenario_block_s
            ...
            if not global_match:
                ...
                if not predecessors or not following_matches:
                    ...
                    new_matches_scenario_nomatch = [
                        elem
                        for elem in matches_scenario_nomatch
                        if elem[1] not in broken_matches
                    ]
                    ...
                    if (
                        [self.node_id_t, self.node_id_c] in matches_scenario_nomatch
