...
class BasePadding(TransformationPass):
        ...
        for node in dag.topological_op_nodes():
            if node in node_start_time:
                ...
                for bit in node.qargs:
                    ...
                    if t0 - idle_after[bit] > 0 and self.__delay_supported(
    ...
    def _pre_runhook(self, dag: DAGCircuit):
        ...
        if self.property_set["time_unit"] == "stretch":
            ...
            raise TranspilerError(
                "Scheduling cannot run on circuits with stretch durations."
            )
    ...
    def _apply_scheduled_op(
        ...
        new_node = dag.apply_operation_back(
            oper, qargs=qubits, cargs=clbits, check=False
        )
