...
class ContextAwareDynamicalDecoupling(TransformationPass):
    ...
    def __init__(
        ...
        *,
        ...
        if not 0 <= skip_dd_threshold <= 1:
            raise ValueError(
                f"skip_dd_threshold must be in [0, 1], but is {skip_dd_threshold}"
            )
        ...
        )  # build once and re-use for performance
    ...
    def run(self, dag: DAGCircuit) -> DAGCircuit:
        ...
        adjacent_delay_blocks = self._collect_adjacent_delay_blocks(
            sorted_delay_events, qubit_map
        )
        ...
        all_delays = (
            set()
        )  # keep a list of all delays to iterate over them easily later
    ...
    def _get_sorted_delays(
        ...
        def is_after_reset(node):
            ...
            return isinstance(predecessor, DAGInNode) or isinstance(
                predecessor.op, Reset
            )
    ...
    def _get_wire_coloring(
        self, dag: DAGCircuit, merged_delay: MultiDelayOp
    ) -> dict[int, int]:
    ...
    def _get_dd_sequence(
        ...
        dd_sequence_duration = sum(
            instruction_durations.get(gate, index) for gate in dd_sequence
        )
    ...
    def _constrain_spacing(self, spacing, slack):
        ...
        unused_slack = slack - sum(
            taus
        )  # unused, due to rounding to int multiples of dt
        ...
        def _combine_delay_blocks(delay_blocks):
            ...
            for doomed_delay_group in doomed:
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    if survivor.active_qubits.intersection(
                        doomed_delay_group.active_qubits
                    ):
                ...
                survivor.events.sort(
                    key=DelayEvent.sort_key
                )  # Maintain sorted event order
            ...
            adjacent_open_delay_blocks = [
                ...
                if any(
                    self._coupling_map.distance(
                        qubit_map[delay_event.op_node.qargs[0]],
                        qubit_map[open_delay_qubit],
                    )
                    <= 1
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            for closed_delay in closed_delay_blocks:
                if len(closed_delay.active_qubits) > 0:
                    logger.debug(
                        "Failed to remove active qubits on closed delay %s.",
                        closed_delay,
                    )
    ...
    def _split_adjacent_block(
        ...
        active_delays = (
            {}
        )  # {window: [group1, group2]} where each group = (index1, index2, ..)
                ...
                if open_group in next_groups:
                    ...
                    if logger.isEnabledFor(logging.DEBUG):
                        ...
                        if len(involved_ops.difference(delay.ops)) > 0:
                            logger.debug(
                                "Not all involved operations are part of the joint delay."
                            )
        ...
        logger.debug(
            "Split adjacent delay block into %s", "\n".join(map(str, merged_delays))
        )
...
class DelayOp:
    ...
    def add_window(self, window: tuple[int, int]):
        ...
        if self.end is None:
            raise ValueError(
                "Cannot add a window if DelayOp.end is None. Please set it."
            )
    ...
    def validate(self, log: bool = True) -> None:
        ...
        for idx, event in enumerate(self.events[:-1]):
            ...
            if event.time == self.events[idx + 1].time:
                ...
                if (event.type, self.events[idx + 1].type) == (
                    EventType.BEGIN,
                    EventType.END,
                ):
                    notify(
