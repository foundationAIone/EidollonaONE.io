        ...
        for qarg, _ in enumerate(dag.qubits):
            for gate in self._dd_sequence:
                if not self.__gate_supported(gate, qarg):
                    ...
                    logger.debug(
                        "No DD on qubit %d as gate %s is not supported on it",
                        qarg,
                        gate.name,
                    )
                    break
    ...
    def _pad(
        ...
        if not self.__is_dd_qubit(dag.qubits.index(qubit)):
            ...
            self._apply_scheduled_op(
                dag, t_start, Delay(time_interval, dag._unit), qubit
            )
            return
        ...
        if len(self._dd_sequence) == 1:
            ...
            if isinstance(next_node, DAGOpNode) and isinstance(
                ...
                op.params = Optimize1qGates.compose_u3(
        ...
        for dd_ind in range(num_elements):
            if dd_ind < len(taus):
                ...
                if tau > 0:
                    self._apply_scheduled_op(
                        dag, idle_after, Delay(tau, dag._unit), qubit
                    )
                    idle_after += tau
