...
class TimeUnitConversion(TransformationPass):
    ...
    * raise error: if they are a mix of SI units and ``'dt'``.
    ...
    def __init__(
        self, inst_durations: InstructionDurations = None, target: Target = None
    ):
    ...
    def run(self, dag: DAGCircuit):
        ...
        if self._durations_provided:
            inst_durations.update(
                self.inst_durations, getattr(self.inst_durations, "dt", None)
            )
        ...
        for node in dag.op_nodes(op=Delay):
            if isinstance(node.op.duration, expr.Expr):
                if any(
                    isinstance(x, expr.Stretch)
                    for x in expr.iter_identifiers(node.op.duration)
                ):
        ...
        for node in dag.op_nodes(Delay):
            ...
            if node._node_id in expression_durations:
                ...
                op.duration = inst_durations._convert_unit(
                    op.duration, op.unit, time_unit
                )
            op.unit = time_unit
