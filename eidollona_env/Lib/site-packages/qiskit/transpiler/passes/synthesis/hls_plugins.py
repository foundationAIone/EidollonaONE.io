...
class MCXSynthesisNDirtyI15(HighLevelSynthesisPlugin):
    ...
    * action_only: when set to ``True``, the method applies only the ``action part``
    ...
    def run(
        ...
        if (
            num_ctrl_qubits >= 3
            and num_dirty_ancillas + num_clean_ancillas < num_ctrl_qubits - 2
        ):
            # This synthesis method is not applicable as there are not enough ancilla qubits
            return None

        decomposition = synth_mcx_n_dirty_i15(
            num_ctrl_qubits, relative_phase, action_only
        )
        return decomposition
...
class MCMTSynthesisNoAux(HighLevelSynthesisPlugin):
    ...
    def run(
        ...
        if high_level_object.num_target_qubits == 1:
            ...
            circuit.append(
                base_gate.control(
                    high_level_object.num_ctrl_qubits, ctrl_state=ctrl_state
                ),
                circuit.qubits,
            )
            ...
            base = QuantumCircuit(
                high_level_object.num_target_qubits, name=high_level_object.label
            )
...
class IntComparatorSynthesisNoAux(HighLevelSynthesisPlugin):
    ...
    def run(
        ...
        return synth_integer_comparator_greedy(
            high_level_object.num_state_qubits,
            high_level_object.value,
            high_level_object.geq,
        )
...
class ModularAdderSynthesisD00(HighLevelSynthesisPlugin):
    ...
    def run(
        ...
        return adder_qft_d00(
            high_level_object.num_state_qubits, kind="fixed", annotated=True
        )
...
class HalfAdderSynthesisD00(HighLevelSynthesisPlugin):
    ...
    def run(
        ...
        return adder_qft_d00(
            high_level_object.num_state_qubits, kind="half", annotated=True
        )
...
class AnnotatedSynthesisDefault(HighLevelSynthesisPlugin):
    ...
    def run(
        ...
        is_inverted = (
            sum(1 for mod in modifiers if isinstance(mod, InverseModifier)) % 2
        )
        ...
        if conjugate_decomp is None:
            ...
            synthesized = self._apply_annotations(
                synthesized_base_op, operation.modifiers
            )
        else:
    ...
    def _apply_annotations(
        circuit: QuantumCircuit, modifiers: list[Modifier]
    ) -> QuantumCircuit:
        """
        ...
        for modifier in modifiers:
            if isinstance(modifier, InverseModifier):
                ...
                controlled_circuit = QuantumCircuit(
                    modifier.num_ctrl_qubits + circuit.num_qubits
                )
                ...
                for inst in circuit:
                    ...
                    controlled_qubits = list(range(0, modifier.num_ctrl_qubits)) + [
                        modifier.num_ctrl_qubits + circuit.find_bit(q).index
                ...
                raise TranspilerError(
                    f"AnnotatedSynthesisDefault: Unknown modifier {modifier}."
                )
...
class WeightedSumSynthesisDefault(HighLevelSynthesisPlugin):
    ...
    def run(
        ...
        required_auxiliaries = (
            high_level_object.num_sum_qubits
            - 1
            + int(high_level_object.num_sum_qubits > 2)
        )
