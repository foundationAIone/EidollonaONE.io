"""Symbolic utilities for the `jurisdiction_data` data bucket."""
...
__all__ = ["JurisdictionDataSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "jurisdiction_data", "module": "jurisdiction_data"},
}


@dataclass(slots=True)
class JurisdictionDataSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> JurisdictionDataSnapshot:
    ...
    if path.exists():
        ...
        return JurisdictionDataSnapshot(bucket="jurisdiction_data", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = JurisdictionDataSnapshot(bucket="jurisdiction_data", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> JurisdictionDataSnapshot:
    ...
    snapshot = JurisdictionDataSnapshot(bucket="jurisdiction_data", context=ctx, signals=signals)
