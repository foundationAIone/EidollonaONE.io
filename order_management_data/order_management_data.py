"""Symbolic utilities for the `order_management_data` data bucket."""
...
__all__ = ["OrderManagementDataSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "order_management_data", "module": "order_management_data"},
}


@dataclass(slots=True)
class OrderManagementDataSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> OrderManagementDataSnapshot:
    ...
    if path.exists():
        ...
        return OrderManagementDataSnapshot(bucket="order_management_data", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = OrderManagementDataSnapshot(bucket="order_management_data", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> OrderManagementDataSnapshot:
    ...
    snapshot = OrderManagementDataSnapshot(bucket="order_management_data", context=ctx, signals=signals)
