"""Symbolic utilities for the `trade_executor_data` data bucket."""
...
__all__ = ["TradeExecutorDataSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "trade_executor_data", "module": "trade_executor_data"},
}


@dataclass(slots=True)
class TradeExecutorDataSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> TradeExecutorDataSnapshot:
    ...
    if path.exists():
        ...
        return TradeExecutorDataSnapshot(bucket="trade_executor_data", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = TradeExecutorDataSnapshot(bucket="trade_executor_data", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> TradeExecutorDataSnapshot:
    ...
    snapshot = TradeExecutorDataSnapshot(bucket="trade_executor_data", context=ctx, signals=signals)
