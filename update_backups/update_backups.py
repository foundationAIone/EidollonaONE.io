"""Symbolic utilities for the `update_backups` data bucket."""
...
__all__ = ["UpdateBackupsSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "update_backups", "module": "update_backups"},
}


@dataclass(slots=True)
class UpdateBackupsSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> UpdateBackupsSnapshot:
    ...
    if path.exists():
        ...
        return UpdateBackupsSnapshot(bucket="update_backups", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = UpdateBackupsSnapshot(bucket="update_backups", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> UpdateBackupsSnapshot:
    ...
    snapshot = UpdateBackupsSnapshot(bucket="update_backups", context=ctx, signals=signals)
