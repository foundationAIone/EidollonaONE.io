"""Symbolic utilities for the `identity_data` data bucket."""
...
__all__ = ["IdentityDataSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "identity_data", "module": "identity_data"},
}


@dataclass(slots=True)
class IdentityDataSnapshot:
    bucket: str
...
def load_snapshot(root: Path | None = None) -> IdentityDataSnapshot:
    ...
    if path.exists():
        ...
        return IdentityDataSnapshot(bucket="identity_data", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = IdentityDataSnapshot(bucket="identity_data", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> IdentityDataSnapshot:
    ...
    snapshot = IdentityDataSnapshot(bucket="identity_data", context=ctx, signals=signals)
