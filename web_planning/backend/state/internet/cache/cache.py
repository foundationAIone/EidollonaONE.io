"""Symbolic utilities for the `web_planning/backend/state/internet/cache` data bucket."""
...
__all__ = ["CacheSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "web_planning/backend/state/internet/cache", "module": "cache"},
}
...
def load_snapshot(root: Path | None = None) -> CacheSnapshot:
    ...
    if path.exists():
        ...
        return CacheSnapshot(bucket="web_planning/backend/state/internet/cache", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = CacheSnapshot(bucket="web_planning/backend/state/internet/cache", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> CacheSnapshot:
    ...
    snapshot = CacheSnapshot(bucket="web_planning/backend/state/internet/cache", context=ctx, signals=signals)
