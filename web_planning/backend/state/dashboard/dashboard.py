"""Symbolic utilities for the `web_planning/backend/state/dashboard` data bucket."""
...
__all__ = ["DashboardSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "web_planning/backend/state/dashboard", "module": "dashboard"},
}
...
def load_snapshot(root: Path | None = None) -> DashboardSnapshot:
    ...
    if path.exists():
        ...
        return DashboardSnapshot(bucket="web_planning/backend/state/dashboard", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = DashboardSnapshot(bucket="web_planning/backend/state/dashboard", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> DashboardSnapshot:
    ...
    snapshot = DashboardSnapshot(bucket="web_planning/backend/state/dashboard", context=ctx, signals=signals)
