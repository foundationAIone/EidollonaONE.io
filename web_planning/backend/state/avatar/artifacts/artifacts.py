"""Symbolic utilities for the `web_planning/backend/state/avatar/artifacts` data bucket."""
...
__all__ = ["ArtifactsSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "web_planning/backend/state/avatar/artifacts", "module": "artifacts"},
}
...
def load_snapshot(root: Path | None = None) -> ArtifactsSnapshot:
    ...
    if path.exists():
        ...
        return ArtifactsSnapshot(bucket="web_planning/backend/state/avatar/artifacts", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = ArtifactsSnapshot(bucket="web_planning/backend/state/avatar/artifacts", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> ArtifactsSnapshot:
    ...
    snapshot = ArtifactsSnapshot(bucket="web_planning/backend/state/avatar/artifacts", context=ctx, signals=signals)
