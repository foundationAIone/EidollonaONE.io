"""Symbolic utilities for the `web_planning/backend/state/avatar` data bucket."""
...
__all__ = ["AvatarSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "web_planning/backend/state/avatar", "module": "avatar"},
}
...
def load_snapshot(root: Path | None = None) -> AvatarSnapshot:
    ...
    if path.exists():
        ...
        return AvatarSnapshot(bucket="web_planning/backend/state/avatar", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = AvatarSnapshot(bucket="web_planning/backend/state/avatar", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> AvatarSnapshot:
    ...
    snapshot = AvatarSnapshot(bucket="web_planning/backend/state/avatar", context=ctx, signals=signals)
