"""Symbolic utilities for the `web_planning/backend/state/memory` data bucket."""
...
__all__ = ["MemorySnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "web_planning/backend/state/memory", "module": "memory"},
}
...
def load_snapshot(root: Path | None = None) -> MemorySnapshot:
    ...
    if path.exists():
        ...
        return MemorySnapshot(bucket="web_planning/backend/state/memory", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = MemorySnapshot(bucket="web_planning/backend/state/memory", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> MemorySnapshot:
    ...
    snapshot = MemorySnapshot(bucket="web_planning/backend/state/memory", context=ctx, signals=signals)
