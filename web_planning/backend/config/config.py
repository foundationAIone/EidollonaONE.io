"""Symbolic utilities for the `web_planning/backend/config` data bucket."""
...
__all__ = ["ConfigSnapshot", "load_snapshot", "refresh_snapshot"]
...
_DEFAULT_CONTEXT = {
    ...
    "metadata": {"bucket": "web_planning/backend/config", "module": "config"},
}
...
def load_snapshot(root: Path | None = None) -> ConfigSnapshot:
    ...
    if path.exists():
        ...
        return ConfigSnapshot(bucket="web_planning/backend/config", context=ctx, signals=sig)
    signals = _EQ.evaluate(_DEFAULT_CONTEXT).to_dict()
    snapshot = ConfigSnapshot(bucket="web_planning/backend/config", context=_DEFAULT_CONTEXT, signals=signals)
    path.write_text(json.dumps(snapshot.to_dict(), indent=2), encoding="utf-8")
    return snapshot


def refresh_snapshot(context_override: Dict[str, Any] | None = None) -> ConfigSnapshot:
    ...
    snapshot = ConfigSnapshot(bucket="web_planning/backend/config", context=ctx, signals=signals)
